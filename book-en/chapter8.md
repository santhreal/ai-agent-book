...
Compressed into a minimal training skeleton, the essential point is not any particular training framework's API but this boundary: **prompt tokens supply no supervision, answer tokens do**.
...
The `-100` here only marks the loss mask; it does not remove the prompt from the model's input. The model still has to read the question in order to learn the answering protocol.
...
The section "Pre-training, SFT, RL: A Three-Stage Panorama" clarified the **essential difference** between SFT and RL. This section answers a more practical question: **Given a specific task, which one should you use?** Some conclusions from the decision framework below will be further validated in subsequent RL experiments (Experiment 8-10, Experiment 8-11). Readers can first form a preliminary judgment and then return to cross-reference after reading the RL section.
...
As for when the two should be chained, the "form first, spirit second" criterion from "Pre-training, SFT, RL: A Three-Stage Panorama" already gives the test: when structured output is unstable, use SFT to make the "form" stand up first, so that the RL reward can be computed at all; when the base model is strong enough to produce acceptable output from the outset, RL can be applied directly.
...
The same mechanism can be written as the Python-style skeleton below. It omits sampling parallelism, KL regularization, and optimizer details, marking only the causal chain from one rollout to a parameter update:
...
This boundary can be written as a very short trajectory-level mask:
...
This is not to say environment feedback is unimportant; the feedback is what computes the reward and the advantage, it simply should not be treated as a target sequence for the policy to reproduce. Tool messages must retain their source markers and stay distinguishable from policy output inside the execution environment.
...
On-Policy Distillation was systematically formulated and popularized by Thinking Machines Lab in 2025[^ch8-10]. It sets out to fix the weaknesses of SFT and RL at the same time: SFT's supervision is dense, but it comes from the **off-policy paths** a teacher or a human walked; when the student makes its own mistakes and enters states the training data never covered, it does not know how to recover. RL lets the student generate its own **on-policy paths**, but a trajectory usually carries only one final reward, so the learning signal is sparse and high-variance.

On-Policy Distillation has the student generate trajectories under its own policy first, then has a stronger teacher give the next-token probability distribution at **every state the student actually visited**. A rollout of length $T$ therefore no longer produces a single 0/1 signal but roughly $T$ sets of token-level supervision; what the teacher's inference consumes is compute, not extra environment interaction. This avoids SFT's distribution mismatch while sharply reducing RL's variance and its number of trials: one expensive sample already teaches "what to do differently at this step," instead of waiting for the task to end and reasoning backward from success or failure.
...
The control flow compresses to this: the student walks its own path, and the teacher supplies a distribution only at the states the student actually visited, rather than replaying an off-policy answer on the student's behalf.
...
This skeleton only serves to distinguish on-policy states from token-level supervision.
...
OPSD can be read as a constrained variant of the skeleton above:
...
1.  **Over-relying on post-training to memorize facts**—factual knowledge should be managed with RAG (dynamically updatable, traceable to a source, and not forgotten through training), while post-training focuses on "how to use knowledge."
...
3.  **Poorly designed reward functions** leading to reward hacking—the model learns to exploit loopholes in the reward for a high score instead of actually completing the task (for example, generating long, meaningless text when only response length is measured). Evaluate the final goal, not an intermediate proxy.
4.  **Ignoring simulation fidelity**—if the simulation is too simplistic (a customer-service script that always replies the same way) or the environment's responses are unrealistic (error messages that do not match production), the resulting policy fails completely in real scenarios. Building a high-fidelity simulation environment can cost more than the training itself.
5.  **Over-training that degrades generalization**—when training loss keeps falling while validation performance worsens, the model is memorizing training details. SFT is especially prone to this and early stopping remains essential; over-optimized RL likewise overfits the policy to the current task distribution.
6.  **Value-function collapse and insufficient exploration**—inaccurate value estimates in PPO bias the advantage computation, showing up as violently oscillating training curves. Too low a temperature or too little randomness traps the Agent in a local optimum.
7.  **Underestimating RL's compute cost**—a task that works well with SFT may need 10–100 times the training time under RL. If the test distribution closely matches training, SFT may already be enough.
8.  **Low-quality training data**—SFT learns the noise and bias in the data directly, freezing errors into parameters; RL may find better strategies through exploration, but if the reward model has a systematic bias it will optimize in the wrong direction.

Core principle: **validate the key assumptions with small-scale experiments before committing large-scale resources**—test with a little data whether SFT can stabilize the format, use a simplified environment to check whether RL converges, and use a small sample to check whether the reward function reflects the real goal. Failing fast is more acceptable than failing at scale.
...
Robust systems generally combine these methods: manage facts and evidence with RAG, experiment quickly with strategies that can be described in language via ICL, encode deterministic processes and hard constraints in program code, and write into parameters through post-training those capabilities that are hard to express in language and need broad generalization. Post-training also enables model distillation—transferring the capability of a large, high-capability model into a cheaper small one.
...
Two further judgments run through the whole chapter and are worth remembering more than any algorithm. First, **data and environment matter more than algorithms**: it is enough to know how to use off-the-shelf RL algorithms; what truly separates teams is the fidelity of the simulation environment and the quality of the training data. When a real environment cannot be built, using a model to simulate the environment (synthesizing tool return values, simulating environment dynamics) is also a viable route—but remember that the simulator's bias is the ceiling on training. Not only can answers be filtered; the task distribution of the training data can itself become an optimization target. In many scenarios, if the SFT data is of sufficiently high quality, you may not need RL at all.

Second, **RL's main bottleneck today is sample efficiency**: On-Policy Distillation expands one rollout's terminal scalar into token-level supervision, and RLVP turns wasted environment feedback into a learnable signal; these are the two most promising directions at present. What they share is taking information that already exists in the environment and the data, but that pure outcome rewards squander, and turning it back into something the model can learn.
