...
def sample_notebook(temp_dir):
    ...
    notebook_data = {
        "cells": [
            ...
            {"id": "cell-2", "cell_type": "markdown", "source": ["# Title"]},
            {
                "id": "cell-3",
                "cell_type": "code",
                "source": ["x = 1\n", "y = 2"],
...
class TestNotebookEditTool:
    ...
    def test_delete_cell_no_new_source(self, system_state, sample_notebook):
        """Test edit_mode=delete without new_source parameter"""
        tool = NotebookEditTool(system_state)
        result = tool.execute(
            {
                "notebook_path": str(sample_notebook),
    ...
    def test_insert_requires_new_source(self, system_state, sample_notebook):
        """Test that insert mode requires new_source"""
        tool = NotebookEditTool(system_state)
        result = tool.execute(
            {"notebook_path": str(sample_notebook), "edit_mode": "insert"}
        )
        assert "error" in result.data
        assert "new_source required" in result.data["error"]

    def test_replace_requires_new_source(self, system_state, sample_notebook):
        """Test that replace mode requires new_source"""
        tool = NotebookEditTool(system_state)
        result = tool.execute(
            {
                "notebook_path": str(sample_notebook),
