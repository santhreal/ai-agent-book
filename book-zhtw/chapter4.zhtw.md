...
原生處理的能力上限最高，Vision Transformer 等編碼器會將不同資料映射到共同的語義空間。文字擷取適合不支援原生多模態的模型，也能在文字為主的 PDF 中節省 token，但會遺失版面、圖表與圖片。當主模型不支援多模態時，可用 `analyze_image`、`analyze_pdf`、`analyze_audio` 等工具把檔案與問題交給專用模型，只把簡短結果保留在上下文中。
