...
實驗 2-8 是個小規模的定性演示，給的是直覺。這套「提前算好、直接查一眼」的做法到底有多大用、邊界又在哪，筆者和合作者用一個專門的基準量化了一遍[^ch2-8]（這套做法有個統一的名字，叫**上下文蒸餾，Context Distillation**——Agent 狀態列是它最日常的形態）。結論：
...
- 最本質的變化是：不帶狀態列時，每次查詢的思考量隨上下文變長而**持續增長**；帶上狀態列後，它變得**基本恆定**。不管上下文堆到多長，模型都只是「瞥一眼」那幾格狀態。

不過，「提前算好」這件事，**做對和做錯是天壤之別**。三條經驗：
...
**二、不要刪掉原始上下文。** 狀態列是對原始上下文的一次**有損投影**——它只提前算了「你預想會被問到」的那些維度。如果狀態列夠用（計數、狀態跟蹤這類任務就是如此），你完全可以把原始記錄整段刪掉、只留狀態列，省下大把 token；可只要有一個問題落到了狀態列沒算過的維度上，只留狀態列的準確率就會斷崖式崩塌。

**三、把狀態列的準確率當成一線生產指標來盯。** 實驗發現：**模型幾乎無條件地相信狀態列**——你寫「打了 3 次」，它就當真是 3 次，既不會偷偷去核對，也不會自己重算。這既是狀態列有效的原因，也意味著狀態列一旦寫錯，錯誤會**原樣**傳進最終答案。這也意味著前面提過的**狀態列投毒**風險值得認真對待。
...
前面已經分析了壓縮的兩個動機（控制長度與提升思考品質）和「上下文學習本質上是檢索」的內部機制。在此基礎上，我們可以提煉出指導具體壓縮策略設計的四條原則。這裡的壓縮服務於當前任務；當多次任務的軌跡需要被離線整理為持久經驗時，則進入第九章討論的持續進化問題。
