...
需要注意的是，同一種機制可以出現在多層——例如基於規則的正規表示式過濾既能篩輸入也能篩輸出，上文按它提供保證的位置歸類。三層不是替代關係而是疊加關係：上層負責把大部分攻擊擋在外面並儘早給出回饋，下層負責在上層失守時守住不可違反的邊界。
...
後面九章會反覆用到同一批結構。它們不屬於某一章，而是同一個約束下的重複解法，因此在這裡一次性命名並給出規範定義；後續各章直接按名呼叫，只說明本章的變體。
...
**最小 diff + 可回滾（Minimal Diff, Reversible）**：每次修改儘量小、帶來源、可單獨回滾，而不是整體重寫。它讓歸因成為可能——出了問題能定位到具體哪一次改動。第三章的知識更新、第五章的程式碼補丁、第九章的 Prompt 與程式更新都遵循這條；本章開頭給出的三條更新路徑（上下文內適應、外部產物更新、引數更新），也正是按可回滾程度從高到低排列的。

這五個模式共享同一個母題：**把判斷從「模型自己說了算」挪到「模型之外的機制說了算」**——審核者在上下文之外，目錄在正文之外，快取在改動之外，保留集在邊界集之外，回滾在提交之外。本章前面給出的三層護欄，就是這個母題在安全上的實例。後文再遇到它們時，本書只標註模式名與本章的差異，不再重新推導。

## 發現迴圈：證據、提案、實驗、回饋

上一節的五個模式是區域性結構。還有一個更大的結構橫跨三章，但因為它被拆開建，很容易被當成三條各自獨立的管線。

第七章要從失敗軌跡裡定位首個錯誤、判定錯誤類別；第三章要把一條新證據變成一個儘量小、有據可依的知識變更；第九章要判斷一次修改是否真的讓系統變好，然後決定發布還是回滾。這三件事用的機制完全不同——歸因靠的是 Rubric 與首錯定位，提案靠的是提議者—審核者，驗證靠的是邊界集 + 保留集與灰度回滾——所以它們**不是同一套機制的三次重複**，把它們的詞彙強行統一反而會掩蓋真正的差別。

真正共同的是它們在同一個迴圈裡各佔一段：

```text
證據（第七章）：失敗軌跡 → 首個錯誤 + 錯誤類別
  → 提案（第三章）：證據 → 最小、可審查、可回滾的一次修改
  → 實驗（第九章）：在邊界集與保留集上實測，灰度發布
  → 回饋：實測結果與新的失敗軌跡回到證據段
```

這個迴圈最近被 Jeff Dean 等人創辦的 Discovery Loop 命名並推向自動化：提出實驗、實現它、評估它、拿到結果，再把結果餵給下一輪，把原本序列的研究過程平行化[^ch1-discovery-loop]。需要說明的是，該公司成立於 2026 年 8 月，目前只公布了使命，尚無公開的技術結果；本書引用它是取其**對迴圈的命名**，而不是把它當作證據——這正是第七章將反覆強調的區分。

把 Agent 系統的這個迴圈和純研究迴圈放在一起看，會發現前者多了兩條約束，而它們恰恰是本書後面大量篇幅在處理的東西。**第一，實驗必須紮根於真實觀測。** 研究迴圈裡的「實驗」可以是一次訓練跑分，Agent 系統裡的「實驗」卻在改動一個正在服務使用者的系統，因此判定必須來自環境的真實狀態（測試是否通過、資料庫最終狀態、工具返回），而不是模型對自己行為的複述。**第二，每次實驗都要同時回答「它改好了什麼」和「它弄壞了什麼」。** 研究迴圈通常只追求指標上升，Agent 系統還必須證明沒有破壞原本正確的行為——這就是上一節「邊界集 + 保留集」存在的理由。

後面三章各自只講自己那一段，不再重述整個迴圈：第三章講提案怎樣才算有據可依，第七章講證據怎樣才算可信，第九章講實驗與回饋怎樣才能長期運轉而不發散。

[^ch1-discovery-loop]: Discovery Loop 由 Jeff Dean、Sanjay Ghemawat、Quoc Le 和 Oriol Vinyals 於 2026 年 8 月 5 日宣布創立，是一家以「自動化機器學習、科學與工程」為使命的公益公司，其公開表述是自動化完整的實驗迴圈、把原本序列的實驗大規模平行化。參見 https://techcrunch.com/2026/08/05/jeff-dean-and-other-top-ai-researchers-are-leaving-google-to-launch-their-own-startup/ 。截至本書寫作時，該公司尚未發表可重現的技術結果。
...
**五個模式貫穿全書**：提議者—審核者、漸進式披露、只增不改、邊界集 + 保留集、最小 diff + 可回滾——它們共享同一個母題，即把判斷從模型自己挪到模型之外的機制上。後續各章按名呼叫，不再重新推導。

**發現迴圈橫跨三章**：證據（第七章的失敗歸因）→ 提案（第三章的知識更新）→ 實驗與回饋（第九章的驗證與發布）。三段用的機制本就不同，共同的不是詞彙而是位置；相比純研究迴圈，Agent 系統的這個迴圈還多兩條約束——實驗必須紮根真實觀測，且每次都要同時回答「改好了什麼」和「弄壞了什麼」。

**安全是架構問題**：護欄、人工干預、對齊（alignment，即讓模型的行為與人類意圖保持一致）——安全問題從第一行程式碼就要考慮，而不是上線前打補丁。護欄按被繞過的難度分為上下文層、執行層與資料層三層，後續各章的安全討論都掛在這個骨架上。
