...
本書的核心公式是 Agent = LLM + 上下文 + 工具。本章聚焦於最佳化 LLM 這個「大腦」——透過後訓練讓模型更好地利用上下文和工具，從而提升整個 Agent 系統的能力。第七章結尾指出，評估體系與模擬環境是後訓練的兩塊基石：評估環境為訓練提供練習場，評估指標為訓練定義目標。本章就建立在這兩塊基石之上，討論如何真正改動模型權重，把能力沉澱進引數。
...
**先建立一張最重要的地圖：現代模型的能力開發通常分為三個階段。** 預訓練打地基，SFT 與 RL 則是根據目標、基礎模型和輸出要求選擇或組合的後訓練階段：
...
3. **強化學習（RL，Reinforcement Learning，即讓模型反覆嘗試、根據結果好壞給獎懲來改進行為，類似訓練小狗：做對了給零食，做錯了不給）**：不再直接模仿標準回答的 token，而是讓模型自己去試，把做得好的行為的機率調高、做得差的調低。當獎勵、資料和環境設計得當時，這一步可以讓模型在**沒見過的情況**下做出更好的決策——也是本章篇幅最大、最需要工程功力的一步。

一個直覺類比：預訓練是 「讀萬卷書」（積累知識），SFT 是 「老師手把手教標準解法」（模仿示範），RL 是 「自己下場做題、根據對錯反覆打磨」（試錯提升）。

**本章有兩條貫穿始終的主線：**

- **主線一：在本章的對照實驗中，SFT 更容易記住示範，而 RL 表現出更好的泛化。** 在 GeneralPoints 和 V-IRL 的相同任務、模型和預算設定下，SFT 對訓練答案過擬合，而 RL 在分佈變化的測試中更容易學到可遷移策略。這是這些實驗條件下測得的結果，不是 SFT 與 RL 的普遍屬性：資料足夠多樣、正則化得當時 SFT 也能泛化，獎勵或環境有偏時 RL 也會過擬合。本章用「SFT 記憶，RL 泛化」概括這些實驗，並在[「預訓練、SFT、RL：三階段全景」一節](#預訓練sftrl三階段全景)解釋兩種最佳化目標為什麼可能產生這種差異。
...
引言給了三階段的地圖，這一節把每一步的機制講透。三個階段用的**資料**、**最佳化目標**、**代價**各不相同，理解它們的異同，是讀懂整章的鑰匙。表 8-1 先給一個總覽，隨後逐項展開。
...
把這件事壓縮成一個最小訓練骨架，關鍵不在某個訓練框架的 API，而在 **prompt token 不提供監督、answer token 提供監督** 這一邊界：
...
這裡的 `-100` 只表示損失遮蔽，不是把 prompt 從模型輸入中刪除；模型仍需閱讀問題，才能學習回答協定。
...
預訓練先提供語言與知識的地基。真正需要解釋的是：**在什麼條件下 SFT 應該放在 RL 之前？**
...
![圖 8-6 OpenAI 訓練正規化演進](images/fig8-6.svg)
...
>
> 本實驗揭示了多模態模型訓練的基本正規化：複用單模態預訓練成果，透過訓練一個輕量投影層實現跨模態對齊——高效且可擴充套件，但投影層的表達能力有限，可能成為跨模態深層理解的瓶頸。同樣的「視覺編碼器 + 投影層 + LLM」骨架再向前延伸一步、讓模型輸出動作，就是第六章將展開的 VLA（視覺-語言-動作）模型。
...
三個預訓練實驗共同揭示了一個規律：在預算受限時，演算法改進與架構創新比單純擴大規模更具價效比。更重要的是，預訓練賦予模型的是描述性知識與語言建模能力，缺乏結構化的指令遵循和任務導向行為——這正是 SFT 需要填補的空白。
...
[「三階段全景」一節](#預訓練sftrl三階段全景)已經講透了 SFT 的本質（換了資料、只在回答上算損失的 「預測下一個詞」）。這一節用四個實驗，看看這套「把穩定對映與協定寫進引數」的機制在不同任務上具體固化了什麼。SFT 的核心價值不在於注入新知識，而在於**固化協定**：把對映關係、互動格式、風格規範寫入引數，使推理時無需冗長提示即可產出符合預期的輸出。通常只需數千到數萬條高質量樣例，即可建立基本的對話能力與指令遵循。
...
[「三階段全景」一節](#預訓練sftrl三階段全景)講清了 SFT 與 RL 的**本質區別**，這一節回答一個更實操的問題：**面對一個具體任務，到底該用哪一個？** 下面的決策框架部分結論會在後續 RL 實驗（實驗 8-10、實驗 8-11）中進一步驗證，讀者可先建立初步判斷，讀完 RL 部分再回來對照。

![圖 8-11 SFT→RL 兩階段訓練流程](images/fig8-11.svg)

**SFT 適用於**格式固化（JSON 輸出、對話風格）、擁有高質量專家示範、訓練與部署環境高度一致的場景。**可以考慮 RL 的場景**則不同：當實際部署環境與訓練環境存在系統性差異，且能構造反映這些差異的可靠獎勵時（比如訓練時卡牌 J/Q/K 都是 10，部署時變成了 11/12/13；或者訓練時用黑色花色，部署時遇到紅色花色），需要探索最佳策略（專家示範本身不一定最佳），或者標註成本過高、無法為每條路徑都提供示範時，可以考慮 RL。

兩者何時該串起來用，[「三階段全景」一節](#預訓練sftrl三階段全景)的「先形後神」已經給出了判據：結構化輸出不穩定時，先用 SFT 把「形」立穩，RL 的獎勵才算得出來；基礎模型足夠強、一上來就能產出合格輸出時，則可以直接做 RL。

兩者各有代價：SFT 樣本效率高、收斂快，泛化效能很大程度上取決於資料的覆蓋範圍與多樣性；RL 可以探索示範中沒有的策略，但樣本效率低且訓練不穩定。如果增加多樣、優質的示範後，新場景表現仍然停滯，並且有能可靠評估這些場景的獎勵與環境，就可以考慮 RL。

實際決策時，可以按以下順序考慮：

1. **先問：需要後訓練嗎？** 如果透過 Harness 工程（最佳化 prompt、工具設計、上下文管理）就能解決問題，不需要訓練模型。大多數 Agent 應用落在這裡。
2. **如果需要訓練：先試 SFT。** 適用於固化輸出格式（JSON schema、API 呼叫格式）、固化協定性知識（術語的用法、輸出格式、流程習慣，即「該怎麼說、怎麼做」）、統一風格（語氣、長度）。但注意 SFT 不適合注入大量事實性知識（「知道什麼」）——那需要繼續預訓練或交給 RAG。SFT 成本低、見效快。
3. **SFT 不夠時：加 RL。** 適用於需要泛化到新場景、需要探索最佳策略、或標註成本過高的情況。如果輸出尚不能穩定滿足獎勵函式要求，可先用 SFT 或約束解碼穩定格式，再應用 RL；如果強基礎模型已經滿足格式要求，也可以直接做 RL。
...
同一機制可以用下面的 Python 風格骨架表示。它省略取樣並行、KL 正則和最佳化器細節，只標出一次 rollout 到引數更新的因果鏈：
...
這不是在說環境回饋不重要；回饋負責計算獎勵和優勢，只是不應被當成策略要復現的目標序列。工具訊息必須保留來源標記，並在執行環境中與策略輸出區分開。
...
### On-Policy Distillation：讓一次 rollout 產生密集監督

On-Policy Distillation（在軌蒸餾）由 Thinking Machines Lab 於 2025 年系統提出並推廣[^ch8-10]。它要同時解決 SFT 和 RL 的短板：SFT 的監督很密集，卻來自教師或人類走過的**離軌路徑**；學生自己犯錯、進入訓練資料沒有覆蓋的狀態時，不知道如何恢復。RL 讓學生自己生成**在軌路徑**，但一條軌跡通常只有一個最終獎勵，學習訊號稀疏且方差很高。

On-Policy Distillation 讓學生先按自己的策略生成軌跡，再讓更強的教師在**學生實際走過的每個狀態**上給出下一個 token 的機率分佈。於是，一條長度為 $T$ 的 rollout 不再只產生一個 0/1 訊號，而能產生約 $T$ 組逐 token 的監督；教師推理消耗的是計算，而不是額外的環境互動。這樣既避免了 SFT 的分佈錯位，又顯著降低了 RL 的方差和試錯次數：一次昂貴的取樣就能學到「這一步該怎麼改」，而不必等任務結束後再從成敗反推。
...
其控制流可以壓縮為：學生先走自己的路，教師只在學生實際訪問過的狀態上提供分佈，不替學生重放一條離軌答案。
...
這裡的 skeleton 只用來區分 on-policy 狀態與逐 token 監督。
...
這一章從預訓練的「預測下一個詞」出發，走了很長一段路：SFT 高效學習格式與協定，結果導向的 RL 在本章的對照實驗中改善了分佈外泛化；多輪任務引入信用分配難題，獎勵設計從結果獎勵延伸到「獎勵結果、約束過程」的路徑訊號，工具使用則帶來組合爆炸。貫穿其中的線索只有一條——模型學到什麼，取決於訓練訊號教了它什麼；而訊號的質量主要由資料和環境決定，不是由演算法決定。
...
4. **忽視模擬保真度**——若模擬過於簡化（客服總按固定模式回覆）或環境響應不真實（錯誤資訊與生產環境不一致），訓練出的策略在真實場景中會完全失效。高保真模擬環境的構建成本可能高於訓練本身。
5. **過度訓練導致泛化下降**——訓練損失持續下降但驗證集效能反而惡化時，模型正在死記訓練細節。SFT 尤其容易出現這個問題，早停仍然至關重要；RL 過度最佳化同樣會導致策略過擬合當前任務分佈。
6. **價值函式崩潰與探索不足**——PPO 中價值估計不準確會導致優勢計算出現偏差，表現為訓練曲線劇烈震盪。溫度引數過低或隨機性不足會使 Agent 陷入區域性最佳。
...
穩健的系統通常組合使用這些方法：用 RAG 管理事實與證據，用 ICL 快速試驗可用語言描述的策略，用程式固化確定性流程與硬約束，再把難以用語言表達且需要廣泛泛化的能力透過後訓練寫入引數。後訓練還可以實現模型蒸餾——把高能力大模型的能力遷移到成本更低的小模型中。

## 本章小結

SFT 和 RL 與其說是競爭關係，不如說是經常按順序組合的方法。在結構化輸出不穩定的設定中，可以先用 SFT 穩定格式，使 RL 獎勵訊號能夠可靠計算，再用 RL 探索策略並改善分佈外表現。「SFT 記憶、RL 泛化」 概括的是本章受控實驗中觀察到的傾向，並不是不受資料、模型、獎勵與環境影響的普遍規律。

還有兩條貫穿全章、比任何演算法都值得記住的判斷。其一，**資料和環境比演算法更重要**：現成的 RL 演算法你會用就行，真正拉開差距的是模擬環境的保真度和訓練資料的質量；造不出真實環境時，用模型模擬環境（合成工具返回值、模擬環境動態）也是一條可行路線，但要記得模擬器的偏差就是訓練的天花板。不僅答案可以篩選，訓練資料的任務分佈本身也可以成為最佳化物件。很多場景下，只要 SFT 的資料質量到位，你甚至不需要做 RL。

其二，**當前 RL 的主要瓶頸是樣本效率**：On-Policy Distillation 把一條 rollout 的終點純量擴充套件為逐 token 監督，RLVP 則把被浪費的環境回饋變成可學習訊號，兩者是目前看起來最有希望的兩個方向。它們的共同點是把環境和資料裡本就存在、卻被純結果獎勵浪費掉的資訊，重新變成模型能學的東西。
