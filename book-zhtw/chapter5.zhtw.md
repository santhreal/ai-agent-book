...
為什麼 Coding Agent 是核心而非其他兩種？因為幾乎所有高效的內容生成最終都要落到程式碼上。PPT 本質上是 OOXML（Office Open XML，微軟推出的辦公文件開放標準）格式的程式碼，Word 文件和 PDF 報告可透過程式碼生成，資料分析和視覺化由 Python 指令碼完成，甚至 GUI 操作中成功的瀏覽器操作序列也可以被固化為可複用的 RPA（機器人流程自動化，Robotic Process Automation）程式碼（Computer Use 本身見第六章，操作序列的固化機制詳見第九章）。Deep Research 的搜尋和資訊綜合可透過程式碼驅動的 Web 請求和解析實現，Computer Use 雖然通用性更強，但成本、延遲和穩定性遠不如直接透過程式碼或 API 來完成相同操作。程式碼生成是效率最高、成本最低、可複用性最強的能力基座。
...
不同模型裁剪這套流程的方式並不相同。有些 Coding 模型會在第一次修改前廣泛閱讀目錄、實作、呼叫端與測試；另一些則只讀少數最可能相關的檔案，立即提交一個修補，再把編譯與測試回饋當作調查的一部分。這種「何時停止收集資訊、開始行動」的閾值，可以在更換 Harness 後繼續跟隨模型，也可以在同一 Harness 內隨模型切換而改變，因此首先是**模型學到的行為策略**，而不只是 Coding 產品的介面風格。Harness 的提示詞、工具與預算仍能放大或抑制它，但不必是它的來源。第七章實驗 7-8 將在固定 Harness 中測量這個差異，第八章再從後訓練角度解釋它可能如何寫入參數。
...
沿著這條「由內而外、最終回到自身」的脈絡閱讀，能更清楚地看到程式碼作為元能力的統一價值。第九章將在此基礎上進一步討論：什麼執行證據應觸發自我修改，以及候選修改如何透過測試、發布與回滾進入新版本。
...
>
> **技術方案**：使用 `PermissionEmbeddedDataObjects` 專案的實作，在 PostgreSQL 之上提供 Python 物件儲存中介層。資料型別宣告權限規則、存取上下文、驗證器、物件關係與反應；每次讀寫都依序通過權限與驗證檢查、持久化與參照完整性處理，以及受控的非同步反應。先在不使用 LLM 的情況下執行確定性的招聘流程示範；可選擇讓模型針對原始 SQL 和 PEDO API 生成對抗性操作，再比較資料庫狀態。關鍵比較不是生成的處理程式是否包含正確的 `if`，而是同一個請求抵達穩定資料層時是否能可靠地被接受或拒絕。
>
> **驗收標準**：有效的招聘流程更新應成功；資料層應拒絕跳過候選人狀態轉換、寫入超出職位範圍的薪資，以及跨租戶讀取。權限、驗證、租戶隔離、參照完整性和反應測試均須通過。實作收錄於第五章配套專案 `permission-embedded-data-objects`。
...
至此，我們完成了全書「建構 Agent」的部分——而程式碼生成正是其中通用性最強的元能力。但一個關鍵問題尚未回答：如何科學地衡量這些設計決策的效果？從下一章開始，全書轉向「評估與進化」：第七章建構從評估環境、資料集到自動化判斷和模型選型的方法論，第七、八章再分別討論參數層和整個 Agent 系統的持續改進。
