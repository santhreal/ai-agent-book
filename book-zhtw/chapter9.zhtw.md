...
>
> **資料與流程**：實驗準備正常退款、虛假承諾、隱私洩漏與過度拒絕四類帶有專家標籤的軌跡。第一層讀取訂單最終狀態與工具日誌，判斷退款或改期是否真實發生；第二層逐步對照業務政策，檢查權限、必要流程、隱私、事實依據與承諾—行動一致性；第三層依照表9-1的 Rubric 評價表達品質與合規變通，並為失敗結論保留證據輪次。預設品質 Judge 使用確定性規則，另提供真實 LLM Judge；無論上層使用哪種模型，結果層與規則層都不交給語言模型猜測。
>
...
>
> 配套實作見 [`trajectory-verifier`](../chapter9/trajectory-verifier/)，預設使用可離線重現的品質 Judge；使用 `--judge llm` 可執行已實作的真實 LLM 驗證器。
