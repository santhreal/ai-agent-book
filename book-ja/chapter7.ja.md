...
最終結果だけでは不十分です。正当かつ許可された操作の割合、ツール引数の意味的な正しさ、経路効率（手順・冗長操作・後戻り）、検索カバレッジ、コストと遅延を測れば、どこで失敗したかが分かります。機密操作・データ漏えい・禁止コンテンツは**ゼロトレランス**とし、seed、UI 変更、API の揺らぎ、古い記憶の干渉に対する堅牢性も評価します。Agent の発話と行動という **trajectory** と、実際のシステム状態という **outcome** の両方を検証します。
