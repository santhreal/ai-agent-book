...
>
> **データと手順**：正常な返金、虚偽の約束、プライバシー漏えい、過剰な拒否という4種類の、専門家ラベル付き軌跡を用意する。第1層は注文の最終状態とツールログを読み、返金または変更が実際に行われたかを判定する。第2層は業務ポリシーと手順ごとに照合し、権限、必須プロセス、プライバシー、事実の根拠、約束と行動の一貫性を検査する。第3層は表9-1の Rubric に従って表現品質とコンプライアンスに沿った代替対応を評価し、失敗判定には証拠となるターンを残す。デフォルトの品質 Judge は決定論的ルールを使い、実際の LLM Judge も提供する。上位層でどのモデルを使う場合でも、結果層とルール層を言語モデルの推測に委ねない。
>
...
>
> 関連実装は [`trajectory-verifier`](../chapter9/trajectory-verifier/) を参照されたい。デフォルトではオフラインで再現可能な品質 Judge を使用し、`--judge llm` を指定すると、実装済みの実際の LLM 検証器を実行できる。
