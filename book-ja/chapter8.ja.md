...
この作業を最小の訓練スケルトンに圧縮すると、要点はどの訓練フレームワークの API かではなく、**prompt の token は監督を与えず、answer の token が監督を与える**という境界にあります。
...
ここでの `-100` は損失マスクを示すだけで、prompt をモデルの入力から取り除くという意味ではありません。モデルは回答のプロトコルを学ぶために、依然として問題を読む必要があります。
...
同じ仕組みは、次の Python 風のスケルトンで表せます。サンプリングの並列化、KL 正則化、オプティマイザの詳細は省き、一度の rollout からパラメータ更新までの因果の連鎖だけを示しています。
...
これは環境のフィードバックが重要でないという意味ではありません。フィードバックは報酬とアドバンテージの計算を担っており、ただ、方策が再現すべき目標系列として扱うべきではないというだけです。ツールのメッセージは出所の印を保持し、実行環境の中で方策の出力と区別できるようにしておかなければなりません。
...
On-Policy Distillation（オンポリシー蒸留）は、2025 年に Thinking Machines Lab が体系的に提示し広めたものです[^ch8-10]。これは SFT と RL の短所を同時に解こうとします。SFT の監督は密ですが、それは教師や人間が歩いた**オフポリシーの経路**から来ます。学生が自分で誤り、訓練データが覆っていない状態へ入ったとき、どう立て直せばよいか分からないのです。RL は学生自身に**オンポリシーの経路**を生成させますが、1 本の軌跡には通常最終報酬が一つあるだけで、学習信号は疎く分散も大きくなります。
...
その制御フローは次のように圧縮できます。学生はまず自分の道を歩き、教師は学生が実際に訪れた状態でのみ分布を提供し、学生の代わりにオフポリシーの答えを再生したりはしません。
...
ここでのスケルトンは、on-policy な状態と token 単位の監督を区別するためだけのものです。
...
表8-4 第 7 章の評価データセットから第 8 章の訓練での使い道への対応
