...
そして本章が**非同期とイベント駆動**から始まるのは、それが読者に最も近いからです。モダリティはまだ純粋なテキストで、変わるのはタイミングだけ。前五章のターン制の世界から踏み出す最初の一歩であり、「ターン制の仮定」という命題が初めて地に足をつける場所でもあります。

## 非同期とイベント駆動：世界のほうから訪ねてくるとき

第四章で論じた知覚・実行・協調の三種類のツールは、いずれも Agent 自身が呼び出すものです——いつ見るか、いつ動くかを決めるのは Agent です。本節はタイミング軸の最も遅い側へ移ります。Agent は数時間、時には数日かかるタスクをどう管理し、いつ届くとも知れない外部イベントにどう応答するのか。これにはイベント駆動の非同期アーキテクチャによる下支えが必要です。第一章の五種類のツールのうち残る二つ——イベントトリガーツールとユーザーコミュニケーションツール——は、まさにこのアーキテクチャに依拠して機能するため、本節でまとめて論じます。
...
[^ch6-async-1]: 約 200 行のオーケストレーションで既製の思考モデルを持続的思考 Agent に変えること、および「訓練信号が持続的思考の有用性を決める」というこの結論については、Li, Bojie and Noah Shi. *Never Stop Thinking: Continuous-Time Language Agents.* 2026（未発表）を参照。
...
>
> マイク、Silero VAD、ローカル Whisper、ストリーミング LLM、Fish S1 TTS を WebSocket で接続し、カスケード基線を作る。保持された単一ターンの実証はメディアとモデルが最後まで動いたことを示すが、並列性や本番負荷のベンチマークではない。[chapter6/live-audio](../chapter6/live-audio/) にコードと受入記録がある。

> **追加プロジェクト：WebRTC で「利用者に電話する」音声 Agent**
>
> PSTN は必須ではない。ブラウザ WebRTC で会話を開始し、不足情報を尋ね、復唱確認し、構造化結果を保存する閉ループを再現できる。外部機関には同じ契約を準拠した PSTN/SIP に置き換える。詳細は [chapter6/phone-agent](../chapter6/phone-agent/) にあり、歴史的な exp9-2 識別子は保持するが本文の番号付き実験ではない。
...
>
> 増加する音声プレフィックスで連続知覚を模擬し、600 ms VAD + Whisper と比較する。canonical run は全ゲートに合格したが期待挙動は 2/6 のみで、8.4–11.3 秒を要し、pause の silence を見落とし noise の cough/laughter を誤分類した。これは失敗モードの検査であり、100–200 ms の真のストリーミングを主張しない。記録は [chapter6/streaming-speech](../chapter6/streaming-speech/) にある。
...
> 表6-1 MiniCPM-o 4.5 ローカルでのエンドツーエンドと自己カスケードの結果（4 件のメカニズム検査であり、ベンチマークではない）
>
>
...
>
> サンプルは小さく、全体の精度や速度は結論できない。完全な証拠は [chapter6/end-to-end-speech](../chapter6/end-to-end-speech/) にある。
...
MGRD は音響特徴に基づく思考を蒸留し、MPS 双脳構造は計画と表現を並列化する。構想側の思考片を表現側が部分回答と結合して音声化するため、全推論を待たずに話せる（図6-11）。統合型は自然だが共同再訓練が必要で、分離型は背景モデルを交換しやすい。

![図6-11：Step-Audio R1 MGRD と MPS 双脳構造](images/fig6-11.svg)
...
![図6-12 Computer Use Agent の知覚-思考-行動ループ](images/fig6-12.svg)
...
Anthropic は完全な対話能力を構成する 3 種類のツールを定義しています（図6-12）。


![図6-13 Computer Use の行動空間](images/fig6-13.svg)
...
>
> 経路 B は本書の付属プロジェクト [`chapter6/computer-use-open-model`](../chapter6/computer-use-open-model/) を使用します。デフォルトではオープンウェイトの Qwen3-VL 32B Instruct で browser-use を駆動し、OpenRouter のホステッド API を利用することも、`OPEN_MODEL_BASE_URL` をセルフホストの vLLM/SGLang または別の互換エンドポイントに向けることもできます。エンドポイントはスクリーンショットを受け取り、ネイティブ JSON Schema をサポートする必要があります。通常の JSON のみをサポートする場合は、schema-in-prompt 互換モードを明示的に有効化できます。
>
...
![図6-14 Set-of-Mark と構造化要素インデックス（browser-use の実装）](images/fig6-14.svg)
...
![図6-15 解像度の整合と双方向の座標スケーリング](images/fig6-15.svg)
...
[^ch6-9]: ゲート制御キーフレーム、必要に応じた書き起こし、フレームを持続的な文字へ語り起こすというこの 3 つの部品、その完全な機構とモデルごとのアブレーションは Li, Bojie and Noah Shi. *Agent-Computer Observation Interfaces Enable Dynamic Computer Use.* arXiv:2606.29472, 2026 を参照。
...
短いチャンクは反応が速い代わりにモデル呼び出しが増え、長いチャンクは滑らかな代わりに古い観測を使ってしまいやすくなります。実験 6-12 はこの種の取捨選択をシミュレータで比較し、実際のハードウェアの安全境界に関わるのは実験 6-11 のほうです。
...
本章が取り払ったのは、前五章がずっと暗黙に置いてきた前提、すなわち Agent と世界が交互に発言するという前提です。

**モダリティ**と**タイミング**という二本の軸に沿って見れば、四つの節は同じ命題が四つの時間スケールで展開したものです。**非同期とイベント駆動**は、観察を「Agent が能動的に取りに行く」から「世界が押し込んでくる」へ、動作を「ターン内に終える」から「先に起動し、後続はイベントで締める」へと拡張します。モダリティは変わらず、変わるのはタイミングだけです。**音声**はスケールをミリ秒まで縮め、カスケード、エンドツーエンド Omni、全二重という三つのパラダイムの進化の本筋は、「交互に話す」から持続的な聴取と発話へ向かい、フォアグラウンドのリアルタイム交互作用とバックグラウンドの深い思考のあいだで分業を成すことでした。**Computer Use** は同じ閉ループを画面上に移し、ボトルネックは「タスクを完了できるか」から操作効率、連続的な視覚理解、動作後の状態確認へと広がりました。**ロボット**はそれを物理世界へ押し進め、動作のチャンク化は滑らかさと反応速度のあいだで取捨を迫られ、最終的に完了したかどうかは、やはり新しい観察によって判定されなければなりません。
...
原語群——起動、セーフポイント、キャンセル、プリエンプション、速い／遅いの分離——も共有します。イベントループにおける「セーフポイントでキャンセル信号を確認する」と、動作チャンク化における「異常を見つけたら残りの動作を捨てて観察し直す」は、5 桁離れた時間スケールにおける同一メカニズムの二度の実装です。フォアグラウンドの速いモデルとバックグラウンドの遅いモデルの分業と、非同期 Agent における「まずタスク ID を返し、後でイベントが締める」も、同じく一つのことの二通りの書き方です。

留意すべきは、これらのメカニズムの多くが今なお工学上の間に合わせだということです。プレースホルダプロトコル、ステータスバーのイベント標識、先回り思考のキャンセルとロールバックは、本質的にはいずれも、モデルの訓練分布に欠けている非同期の経験をオーケストレーションロジックで補っているにすぎません。モデル側の進展はその一部をパラメータへ取り込みつつあります——交互作用モデルは割り込みと差し込み発話をモデル内部に作り込み、持続的思考は「聞きながら考える」を次世代モデルを待たずに可能にしました——が、訓練コーパスがターン制中心であり続ける限り、この補償層は消えず、モデル能力の進展とともに新しい境界へ移り続けるだけです。
