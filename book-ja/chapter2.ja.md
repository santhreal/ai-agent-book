...
実験 2-8 は、直感を得るための小規模な定性的デモです。この「先に計算して、直接ひと目で確認する」方法がどれほど役立ち、どこに限界があるのかを、筆者らは専用のベンチマークで定量化しました[^ch2-8]（この方法の総称は **コンテキスト蒸留、Context Distillation** であり、Agent ステータスバーはその日常的な形です）。結論は次のとおりです。

- **事前計算されたステータスバー**を与えると、**弱いモデルは精度を取り戻します**。最も弱いモデル群では精度が 40〜54 パーセントポイント向上し、ローカルの 2B モデルがこの種のタスクで、ステータスバーなしの最先端モデルに並ぶことさえありました。
- **強いモデルはもともと正答でき、節約できるのは効率です**。同じステータスバーによって、クエリごとの思考量、レイテンシ、コストがそれぞれおよそ 1 桁減りました（思考 token は 80〜90% 以上削減）。
- 最も本質的な変化は、ステータスバーがないとクエリごとの思考量がコンテキストの長さに応じて**増え続ける**のに対し、ステータスバーがあると**ほぼ一定**になることです。コンテキストがどれほど長くても、モデルは数個の状態欄を「ひと目見る」だけです。


ただし、「事前計算」は正しく行うか誤って行うかで大きく異なります。教訓は 3 つです。
...
**二、元のコンテキストを削除しないこと。** ステータスバーは元のコンテキストの**非可逆な射影**であり、「質問されると予想した」次元だけを事前に計算します。カウントや状態追跡のようにステータスバーだけで足りるタスクなら、元の記録を削除して token を大幅に節約できます。しかし、ステータスバーが計算していない次元を問われると、それだけを残した場合の精度は急落します。

**三、ステータスバーの精度を最重要の本番指標として監視すること。** 実験では、**モデルはステータスバーをほぼ無条件に信頼する**と分かりました。「3 回電話した」と書けば、確認も再計算もせず 3 回だと受け取ります。これがステータスバーの有効性の理由である一方、誤りも**そのまま**最終回答へ伝わります。したがって、前述した**ステータスバー汚染**のリスクには注意が必要です。
...
前ですでに圧縮の 2 つの動機（長さの制御と思考の質の向上）と「文脈内学習は本質的に検索である」という内部メカニズムを分析しました。これにもとづき、具体的な圧縮戦略の設計を導く 4 つの原則を精錬できます（第 9 章では Claude Code が記憶の固定化の隠喩をどう直接、周期的なオフラインの記憶統合システムに工学化するかを論じます）。
