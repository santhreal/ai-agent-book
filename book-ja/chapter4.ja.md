...
ネイティブ処理は能力の上限が最も高く、Vision Transformer などのエンコーダーで異なるデータを共通の意味空間へ写像します。テキスト抽出はネイティブ非対応モデルや文章中心の PDF でトークンを節約できますが、レイアウト・図表・画像を失います。主モデルが非マルチモーダルなら、`analyze_image`、`analyze_pdf`、`analyze_audio` などのツールでファイルと質問を専門モデルへ渡し、短い結果だけをコンテキストに残せます。
