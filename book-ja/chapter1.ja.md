...
注意すべきは、同じ仕組みが複数の層に現れうることです——ルールベースの正規表現フィルターは入力も出力もふるいにかけられます——上記はそれが保証を提供する位置で分類しています。3 層は代替関係ではなく積み重ねの関係です。上の層は大半の攻撃を外で食い止め、早期にフィードバックを返す役割を負い、下の層は上が破られたときに侵してはならない境界を守る役割を負います。

分類器型ガードレールの代表的な産業実践が Anthropic の Constitutional Classifiers です[^ch1-3]。中核となる仕組みは三つあります。第一に**ルール駆動**——何を許し何を禁じるかを自然言語で明記した「憲法」から合成訓練データを生成し、入力用・出力用の分類器を訓練します。第二に**文脈を合わせた判定**——新世代のシステムはユーザーの問いとモデルの回答を並べて検査します。回答だけを見れば何の問題もない（「食品香料の使い方」など）ものでも、問いと突き合わせて初めて「食品香料」が化学試薬の隠語だと分かる場合があるからです。第三に**二段階のふるい分け**——まず極めて軽量なプローブ（モデル内部の活性を直接読み取る、ほぼゼロコスト）が全会話を検査し、疑わしいものだけをより強い分類器に回して再審査します。即座に拒否しないため、第一段階で誤検知が多めでもユーザー体験を損なわず、全体のコストも大きく下がります。
...
これ以降の九つの章は、同じひとまとまりの構造を繰り返し使います。それらはどれか一つの章に属するものではなく、同じ制約のもとでの反復的な解法です。ですからここで一度だけ名前を与え、規範的な定義を示します。以降の各章は名前で呼び出し、その章での差分だけを述べます。
...
**最小 diff + ロールバック可能（Minimal Diff, Reversible）**：変更はできるだけ小さく、由来を伴い、単独でロールバックできるようにし、全体を書き直さないこと。これが帰属を可能にします——問題が起きたとき、どの変更かを特定できるのです。第 3 章の知識更新、第 5 章のコードパッチ、第 9 章のプロンプトとプログラムの更新はいずれもこれに従います。本章の冒頭で示した三つの更新経路（コンテキスト内適応、外部生成物の更新、パラメータ更新）も、ロールバックしやすい順に並んでいます。

これら五つのパターンは同じ主題を共有します。すなわち、**判断を「モデル自身が決める」から「モデルの外の仕組みが決める」へ移すこと**——審査者はコンテキストの外に、目次は本文の外に、キャッシュは変更の外に、保持集合は境界集合の外に、ロールバックはコミットの外にあります。本章の前半で示した 3 層ガードレールは、この主題を安全に当てはめた例です。以降でこれらに出会うとき、本書はパターン名とその章での差分を示すだけで、再び導出することはしません。

## 発見ループ：証拠・提案・実験・フィードバック

前節の五つのパターンは局所的な構造です。もう一つ、三つの章にまたがるより大きな構造があります。ただしそれは分割して組み立てられているため、互いに独立した三本のパイプラインだと誤解されやすいのです。

第 7 章は失敗した軌跡から最初の誤りを特定し、その種類を判定しなければなりません。第 3 章は一つの新しい証拠を、できるだけ小さく根拠のある知識の変更へと変えなければなりません。第 9 章はある変更が本当にシステムを良くしたかを判断し、公開するか巻き戻すかを決めなければなりません。この三つが使う仕組みはまったく異なります——帰属はルーブリックと初回誤りの特定に、提案は提案者—審査者に、検証は境界集合・保持集合とカナリアリリースおよびロールバックに依拠します。ですからこれらは**同じ仕組みの三度の繰り返しではありません**。語彙を無理に統一すれば、かえって本質的な違いが覆い隠されてしまいます。

真に共通するのは位置です。三つは同じループの各一区間を占めています。

```text
証拠（第 7 章）：失敗した軌跡 → 最初の誤り + 誤りの種類
  → 提案（第 3 章）：証拠 → 最小で、審査でき、巻き戻せる一つの変更
  → 実験（第 9 章）：境界集合と保持集合の上で実測し、カナリアで公開する
  → フィードバック：実測結果と新たな失敗軌跡が証拠の区間へ戻る
```

このループは最近、Jeff Dean らが創業した Discovery Loop によって名づけられ、自動化へ押し進められています。実験を提案し、必要なものを実装し、評価し、結果を次の周回へ送り、これまで直列だった過程を並列化する、というものです[^ch1-discovery-loop]。ただしはっきり述べておくべきことがあります。同社は 2026 年 8 月の設立で、現時点では使命を公表しただけであり、公開された技術的成果はありません。本書がこれを引くのは**ループへの命名**のためであって、証拠としてではありません——まさに第 7 章が繰り返し強調する区別です。

Agent システムのこのループを純粋な研究ループと並べると、前者には二つの制約が加わることが分かります。そしてそれこそ、本書の残りの多くが扱っている事柄です。**第一に、実験は真の観測に根ざさねばなりません。** 研究ループの「実験」は訓練の一走行でありえますが、Agent システムの「実験」は現にユーザーへ提供中のシステムを書き換えます。ですから判定は環境の実際の状態——テストが通るか、データベースの最終状態、ツールの返り値——から来なければならず、モデル自身による自分の行動の言い直しであってはなりません。**第二に、どの実験も「何を直したか」と「何を壊したか」の両方に答えねばなりません。** 研究ループはたいてい指標の上昇だけを求めますが、Agent システムはもともと正しかった振る舞いを壊していないことも示さねばなりません。前節の境界集合と保持集合が存在する理由がこれです。

続く三章はそれぞれ自分の区間だけを扱い、ループ全体を繰り返し述べることはしません。第 3 章は提案がどうあれば根拠があると言えるか、第 7 章は証拠がどうあれば信頼できるか、第 9 章は実験とフィードバックがどうすれば発散せず長期に回り続けるかを論じます。

[^ch1-discovery-loop]: Discovery Loop は 2026 年 8 月 5 日に Jeff Dean、Sanjay Ghemawat、Quoc Le、Oriol Vinyals によって設立が発表された公益法人で、機械学習・科学・工学の自動化を使命として掲げています。公開されている説明は、完全な実験ループを自動化し、従来直列だった実験を大規模に並列化するというものです。https://techcrunch.com/2026/08/05/jeff-dean-and-other-top-ai-researchers-are-leaving-google-to-launch-their-own-startup/ を参照。本書執筆時点で、再現可能な技術的成果は公表されていません。
...
**5 つのパターンが本書を貫く**：提案者—審査者、漸進的開示、追記のみ、境界集合 + 保持集合、最小 diff + ロールバック可能。いずれも同じ主題——判断をモデル自身からモデルの外の仕組みへ移すこと——を共有します。以降の各章は名前で呼び出し、再び導出することはしません。

**発見ループは三章にまたがる**：証拠（第 7 章の失敗帰属）→ 提案（第 3 章の知識更新）→ 実験とフィードバック（第 9 章の検証と公開）。三つが使う仕組みはもともと異なり、共通するのは語彙ではなく位置です。純粋な研究ループと比べ、Agent システムのループには二つの制約が加わります——実験は真の観測に根ざすこと、そして毎周「何を直し、何を壊したか」の両方に答えることです。

**安全はアーキテクチャの問題である**：ガードレール、人間の介入、アラインメント（alignment、すなわちモデルの振る舞いを人間の意図と一致させ続けること）。安全の問題は最初の 1 行のコードから考えるべきであって、リリース前にパッチを当てるものではありません。ガードレールは迂回されにくさの順にコンテキスト層・実行層・データ層の 3 層に分かれ、以降の各章の安全に関する議論はこの骨格に掛かります。
