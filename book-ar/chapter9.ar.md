...
>
> **البيانات والإجراءات:** جهّز مسارات صنّفها خبراء وتغطي أربع فئات: استردادًا عاديًا، ووعدًا كاذبًا، وكشفًا للخصوصية، ورفضًا مفرطًا. تقرأ الطبقة الأولى الحالة النهائية للطلب وسجلات الأدوات لتتحقق من حدوث الاسترداد أو إعادة الجدولة فعلًا. وتراجع الطبقة الثانية كل خطوة وفق سياسات العمل، بما فيها الصلاحيات والإجراءات الإلزامية والخصوصية والاستناد إلى حقائق والاتساق بين الوعد والفعل. أما الطبقة الثالثة فتقيّم جودة اللغة والبدائل المسموح بها وفق الجدول 9-1، وتحتفظ بالمقاطع ذات الصلة دليلًا على كل إخفاق. يستخدم مقيّم الجودة الافتراضي قواعد حتمية، مع إتاحة مقيّم حقيقي قائم على نموذج لغوي أيضًا. وأيًا كان مقيّم الطبقة العليا، فلا ينبغي ترك النتيجة أو الالتزام بالقواعد لتخمين النموذج.
>
...
وتقوم بنية **Voyager** على ثلاث آليات مترابطة: (1) **مولد المنهج التلقائي (Automatic Curriculum Generator)** الذي يقترح هدفًا تاليًا مناسب الصعوبة استنادًا إلى المخزون والبيئة والمهارات المكتسبة لمنع الاستكشاف العشوائي؛ (2) **مكتبة المهارات (Skill Library)** التي تحفظ البرامج الناجحة في صورة شفرة برمجية قابلة للاسترجاع والتركيب؛ (3) **آلية التوجيه التكرارية (Iterative Prompting Mechanism)** التي تعيد ملاحظات البيئة وأخطاء التنفيذ ونتائج التحقق الذاتي إلى جولة توليد الشفرة التالية حتى تكتمل المهمة بالفعل. وبالمقارنة مع خطوط الأساس، جمعت Voyager عناصر فريدة أكثر بمقدار 3.3 مرة، وقطعت مسافة أكبر بمقدار 2.3 مرة، وبلغت معالم رئيسية في شجرة التقنية بسرعة وصلت إلى 15.3 مرة.
...
| **مكاسب المهام المحجوبة (Held-out)** | هل يتحسن النظام ككل في المهام التي لم تُعرض عليه أثناء عملية التطور؟ | معدلات النجاح والجودة والتكلفة في بيئة مستقلة |
