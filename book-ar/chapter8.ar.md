...
قدمت لك المقدمة خريطة المراحل الثلاث؛ يعمل هذا القسم من خلال آليات كل منها. تختلف المراحل الثلاث في **البيانات**، و**أهداف التحسين**، و**التكاليف**. إن فهم أوجه التشابه والاختلاف بينهما هو مفتاح الفصل بأكمله. ويقدم الجدول 8-1 نظرة عامة؛ التفاصيل تتبع.

جدول 8-1 المراحل الثلاث لتكوين قدرات النموذج
...
وإذا ضغطنا هذا في أصغر هيكل تدريب، فالجوهر ليس واجهة برمجة إطارٍ بعينه، بل هذا الحد: **رموز الموجّه لا تقدّم إشرافاً، ورموز الإجابة تقدّمه**.
...
و`-100` هنا تشير إلى قناع الخسارة فقط، ولا تحذف الموجّه من مدخلات النموذج؛ فالنموذج ما زال بحاجة إلى قراءة السؤال كي يتعلّم بروتوكول الإجابة.
...
ويمكن تمثيل الآلية نفسها بالهيكل التالي على نمط Python. وهو يغفل توازي أخذ العينات وتنظيم KL وتفاصيل المحسِّن، ولا يبيّن سوى سلسلة السببية من rollout واحد إلى تحديث المعاملات:
...
ويمكن كتابة هذا الحدّ كقناع قصير جداً على مستوى المسار:
...
وهذا لا يعني أن تغذية البيئة الراجعة غير مهمة؛ فهي المسؤولة عن حساب المكافأة والأفضلية، لكن لا ينبغي معاملتها كمتتالية هدف على السياسة إعادة إنتاجها. ويجب أن تحتفظ رسائل الأدوات بعلامات مصدرها، وأن تظل قابلة للتمييز عن مخرجات السياسة داخل بيئة التنفيذ.
...
طرح مختبر Thinking Machines Lab أسلوب On-Policy Distillation ونشره على نحو منهجي عام 2025[^ch8-10]. وهو يسعى إلى معالجة نقطتي ضعف SFT وRL معاً: فإشراف SFT كثيف، لكنه يأتي من **مسارات off-policy** سلكها معلّم أو إنسان؛ وحين يخطئ الطالب بنفسه ويدخل حالات لم تغطّها بيانات التدريب لا يعرف كيف يتعافى. أما RL فيجعل الطالب يولّد **مساراته on-policy**، لكن المسار الواحد لا يحمل عادةً سوى مكافأة نهائية واحدة، فتكون إشارة التعلّم متفرّقة وعالية التباين.
...
ويمكن ضغط تدفّق التحكّم هكذا: يسلك الطالب طريقه أولاً، ولا يقدّم المعلّم توزيعاً إلا عند الحالات التي زارها الطالب فعلاً، ولا يعيد تشغيل إجابة off-policy نيابةً عنه.
...
والهيكل هنا لا يفيد إلا في التمييز بين حالات on-policy والإشراف على مستوى الرمز.
...
ويمكن قراءة OPSD بوصفه صيغة مقيَّدة من الهيكل السابق:
...
جدول 8-4 تقابل مجموعات تقييم الفصل السابع مع استخدامات التدريب في الفصل الثامن
