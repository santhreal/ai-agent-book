...
On-Policy Distillation（在轨蒸馏）由 Thinking Machines Lab 于 2025 年系统提出并推广[^ch8-10]。它要同时解决 SFT 和 RL 的短板：SFT 的监督很密集，却来自教师或人类走过的**离轨路径**；学生自己犯错、进入训练数据没有覆盖的状态时，不知道如何恢复。RL 让学生自己生成**在轨路径**，但一条轨迹通常只有一个最终奖励，学习信号稀疏且方差很高。
