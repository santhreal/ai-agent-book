...
四节共享同一套原语——**唤醒、安全点、取消、抢占、快慢分离**——只是参数和失败形态不同。异步事件驱动里的“在安全点检查取消信号”，和机器人动作分块里的“发现异常就丢弃剩余动作、重新观察”，是同一个机制在相差五个数量级的时间尺度上的两次实现。看到这层同构，比记住任何单个场景的技术细节都重要。

## 异步与事件驱动：当世界主动找上门

第四章讨论的感知、执行、协作三类工具都由 Agent 主动调用——是它决定什么时候去看、什么时候动手。本节转向时机轴上最慢的那一端：Agent 如何管理耗时数小时甚至数天的任务，又如何响应随时可能到达的外部事件？这需要事件驱动的异步架构来支撑；第一章五类工具中剩下的两类——事件触发工具与用户沟通工具——正是依托这一架构发挥作用的，因此也放在本节一并讨论。
...
异步与事件驱动把时机轴推到了“世界可以在任意时刻唤醒 Agent”，但它仍然留给模型一个宽裕的前提：事件到达之后，模型可以从容地想完再回应。接下来三节要逐步收紧的正是这个前提——当环境变化的速度逼近甚至超过模型的生成速度时，“想完再说”本身就变成了不可接受的延迟。第一个这样的场景是语音。
...
>
> 表6-1 MiniCPM-o 4.5 本地端到端与自级联结果（4 条机制检查，不是 benchmark）
>
> | 任务类型 | 端到端 | 自级联 | 观察 |
> | --- | ---: | ---: | --- |
...
MPS 让构思脑持续产出思考片段，表达脑收到片段后结合已有回复立即生成语音。两者以流水线方式并行，因此不必等完整思考结束才让用户听到第一句话（图6-11）。


![图6-11 Step-Audio R1 MGRD 与 MPS 双脑架构](images/fig6-11.svg)
...
Computer Use（也称 GUI 自动化 Agent）让 AI 像人类一样通过观察屏幕、操作鼠标键盘来使用软件——比如打开浏览器搜索信息、在表格软件中填写数据、或在系统设置中调整配置。其核心是一个**感知-思考-行动**的循环（图6-12）：
...
![图6-12 Computer Use Agent 的感知-思考-行动循环](images/fig6-12.svg)
...
Anthropic 的参考实现把完整交互能力分成三类工具（图6-13）。这是一个清晰的动作空间设计，但不是模型供应商必须遵守的私有协议：只要 Harness 能把同样的截图、动作约束和执行结果转换成目标模型支持的消息与结构化输出，Claude、开放权重视觉模型和自托管端点都可以驱动同一个感知-思考-行动循环。


![图6-13 Computer Use 动作空间](images/fig6-13.svg)
...
当界面本身能提供结构化信息时，标注可以做得更精确。现代网页在渲染之前就已经定义了完整的元素结构（DOM 树）和语义角色（哪个是按钮、哪个是输入框），无障碍接口（Accessibility Tree）为许多桌面应用提供了类似的信息。与其让分割模型在像素里猜“哪个区域是按钮”，不如直接问界面本身“你有哪些可以点击的元素？”。以 browser-use 项目为代表的 Web Agent 方案正是这样做的：从 DOM 中枚举可交互元素并编号，可以看作 SoM 思想在 Web 上的结构化实现（图6-14）。流程分四步：
...
![图6-14 Set-of-Mark 与结构化元素索引（browser-use 实现）](images/fig6-14.svg)
...
在坐标预测方案中，模型对坐标的理解高度依赖训练时使用的分辨率（图6-15）。Claude 训练使用 XGA（1024x768）、WXGA（1280x800）、FWXGA（1366x768），如果输入的截图分辨率不匹配，模型预测的坐标就会系统性地偏移——就像在小地图上量距离然后直接用到大地图上一样。因此，需要在工具层实现双向坐标缩放机制，而且要**按宽高比选目标分辨率**，避免非等比拉伸把画面压变形、连带把坐标判断也带偏。例如，真实屏幕分辨率为 2560×1440（16:9），就该在 Claude 支持的三档里挑一个宽高比同样接近 16:9 的目标——FWXGA（1366×768）最匹配。截图时把屏幕等比缩放到 1366×768 送入模型；模型输出点击坐标 (683, 384) 后，反向映射为真实坐标 (683×2560/1366, 384×1440/768) ≈ (1280, 720)。反过来，若硬把 16:9 拉伸进 4:3 的 1024×768，画面会被横向压扁，模型预测的坐标就会系统性偏移。


![图6-15 分辨率匹配与双向坐标缩放](images/fig6-15.svg)
...
[^ch6-9]: 门控关键帧、按需转写、把帧叙述成持久文字这三个部件，完整机制与逐模型消融见 Li, Bojie and Noah Shi. *Agent-Computer Observation Interfaces Enable Dynamic Computer Use.* arXiv:2606.29472, 2026.
...
> **阅读提示**：本节始终使用同一个任务——“把红色杯子放进托盘，把黄色废纸放进垃圾盒，最后重新观察并确认桌面状态”。实验 6-9 和 9-9 是 XLeRobot 真机实验，需要机械臂、标定、急停装置和现场观察员；实验 6-10、9-10 和 9-11 是对应的本地 GPU 实验。真机与模拟会明确分开报告，但任务目标、动作语义和成功条件保持一致。
...
>
> 验收标准与实验 6-9 完全相同：杯子位于托盘内、废纸位于垃圾盒内、机械臂回到安全姿态且没有碰撞或越界。区别在于，自主实验的任务语义必须来自模型观察，真实动作必须来自工具调用，最终状态必须由新观察确认；人只能负责启动、急停和安全监护，不能在中途代替 Agent 完成动作。这样，实验 6-9 与 9-9 才能直接比较“同一硬件、同一任务，人类闭环与模型闭环之间还差什么”。
...
RT-2 和 OpenVLA 把连续动作切成离散的 token，再像生成文字一样逐个输出；π₀ 则代表另一条路线，直接生成连续、平滑的动作轨迹。两种方法没有简单的高下之分：离散 token 更容易和语言模型结合，连续轨迹通常更适合表达平滑运动。真正的取舍在于动作应该怎样表示，而不只是模型大小。[^ch6-15]
...
短 chunk 反应更快但增加模型调用，长 chunk 更平滑但更容易使用过时观察；实验 6-12 在模拟器中比较这类取舍，实验 6-11 才涉及真实硬件安全边界。
...
本章撤掉的是前五章一直默认的那个前提：Agent 与世界轮流发言。

顺着**模态**和**时机**两根轴看，四节内容其实是同一个命题在四个时间尺度上的展开。**异步与事件驱动**把观察从“Agent 主动去取”扩展为“世界主动推来”，把动作从“回合内做完”扩展为“先发起、后续靠事件收尾”，模态没变，变的只有时机。**语音**把尺度压到毫秒，级联、端到端 Omni 与全双工三种范式的演进主线，就是从“轮流说话”逐步走向持续听说，并在前台实时交互与后台深度思考之间做出分工。**Computer Use** 把同一个闭环搬到屏幕上，瓶颈已从“能否完成任务”扩展到操作效率、连续视觉理解和动作后的状态确认。**机器人**则把它推到物理世界，动作分块在平滑性与反应速度之间取舍，而最终是否完成，仍必须由新的观察来判定。
...
也共享同一组原语——唤醒、安全点、取消、抢占、快慢分离。事件循环里“在安全点检查取消信号”，与动作分块里“发现异常就丢弃剩余动作、重新观察”，是同一个机制在相差五个数量级的时间尺度上的两次实现；前台快模型加后台慢模型的分工，与异步 Agent 中“先返回任务 ID、稍后由事件收尾”，同样是一件事的两种写法。

需要留意的是，这些机制目前多数仍是工程上的权宜之计。占位符协议、状态栏事件标记、抢先思考的取消与回滚，本质上都是在用编排逻辑弥补模型训练分布中缺失的异步经验。模型侧的进展正在把其中一部分收进参数——交互模型把打断和插话做进了模型内部，持续思考让“边听边想”不必等待下一代模型——但只要训练语料仍以回合制为主，这层补偿就不会消失，只会随着模型能力的推进不断迁移到新的边界上。
