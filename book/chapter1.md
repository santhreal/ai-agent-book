...
先看最小运行骨架。它说明的是**机制如何运行**：Model 只负责决定下一步，Harness 负责组装上下文、校验并执行工具，Environment 负责产生真实状态变化和观察。本书后续也沿用 Python 风格伪代码；伪代码不可以直接运行，也不对应某个 SDK。具体的可执行代码在本书配套代码仓库中。
...
前面几节建立了 **Agent = LLM + 上下文 + 工具** 的最小工程公式，并通过图1-1 明确了 Agent 与 Environment 的边界。从 Harness 工程的视角看，可以把 LLM 抽象为核心组件 Model，把 Agent 边界内负责支撑模型运行、并中介模型与环境交互的代码、配置和服务统称为 Harness。两个视角并非替代关系，而是不同抽象层次上对同一 Agent 实现的描述。之所以换用更通用的 “Model” 一词，是因为 Harness 工程的原则适用于任何具备推理和工具调用能力的模型，不限于某种特定模型类型。Harness 的核心是原公式中的“上下文管理 + 工具接口”，再加上三层保障机制：**约束**（限定 Agent 能做什么、不能做什么）、**验证**（检查 Agent 做得对不对）和**纠正**（做错了怎么补救）。
...
五个功能构成一个闭环：上下文与工具让 Agent “能做事”——理解任务并采取行动；约束预防错误，验证发现偏差，纠正闭合循环，三者共同让 Agent “不做错事”。它们不是独立于上下文和工具之外的东西，而是确保上下文和工具在生产环境中可靠运转的工程实践。缺少任何一个环节，系统都会出现可靠性缺口。而在 Agent 产品的成熟度曲线上，两类功能的重要性是不对称的。
...
选择框架时，关键考量不在于框架本身的复杂度，而在于它能否以最小的抽象层让你专注于业务逻辑。

编排模式解决的是上下文与工具的组织问题。但光能做事还不够，还要确保做得对、做得安全——这就落到了约束、验证与纠正在实践中最核心的手段：护栏。
...
**数据层**护栏管的是**世界最终能被改成什么样**，把“谁能对哪条数据做什么”交给一层稳定的、经过人类审查的机制强制执行：数据库的行级安全策略、约束与校验器、受控视图与存储过程，以及由受信任运行时绑定、无法被伪造的访问上下文。这一层的价值恰恰在于它不依赖上面两层是否正确——即使提示注入得手、生成的代码完全漏写了权限判断，越权操作仍会在数据层被拒绝。第五章会以动态生成软件为例展开这一层。

需要注意的是，同一种机制可以出现在多层——例如基于规则的正则过滤既能筛输入也能筛输出，上文按它提供保证的位置归类。三层不是替代关系而是叠加关系：上层负责把大部分攻击挡在外面并尽早给出反馈，下层负责在上层失守时守住不可违反的边界。
...
**先说清楚两个公式的关系，免得读者记两套骨架。** 全书的结构骨架只有一个，就是引言和后记反复使用的 **Agent = LLM + 上下文 + 工具**：第二至六章构建，第七至九章评估与进化，第十章协作。**Agent = Model + Harness** 不是与它并列的另一套划分，而是同一件事的生产形态展开——它把"上下文"和"工具"两项展开成上下文管理、工具接口、约束、验证、纠正五项职责，因此它是**「构建」这一块内部的一个透镜**，而不是覆盖全书十章的目录。
...
后面九章会反复用到同一批结构。它们不属于某一章，而是同一个约束下的重复解法，因此在这里一次性命名并给出规范定义；后续各章直接按名调用，只说明本章的变体。

**提议者—审核者（Proposer-Reviewer）**：产出与评判由两个不共享上下文的角色分别承担，评判方看到的是产物本身——渲染结果、测试输出、结构化的调用参数——而不是产出方的推理过程。它成立的前提是**自审不可靠**：同一个上下文里的模型既想不到自己没想到的，也很难判断自己是否已被注入。第三章用它更新知识，第四章用它做工具调用的事前审批与事后验证（Sidecar 是它的一个只读变体），第五章的 PPT、视频与日志三个实验都以它为骨架，第七章用它评估 UI，第九章用它审核更新提案，第十章讨论了它在对等协作中的形态，以及为什么不能让同一个 Agent 自审。
...
**最小 diff + 可回滚（Minimal Diff, Reversible）**：每次修改尽量小、带来源、可单独回滚，而不是整体重写。它让归因成为可能——出了问题能定位到具体哪一次改动。第三章的知识更新、第五章的代码补丁、第九章的 Prompt 与程序更新都遵循这条；本章开头给出的三条更新路径（上下文内适应、外部产物更新、参数更新），也正是按可回滚程度从高到低排列的。

这五个模式共享同一个母题：**把判断从"模型自己说了算"挪到"模型之外的机制说了算"**——审核者在上下文之外，目录在正文之外，缓存在改动之外，保留集在边界集之外，回滚在提交之外。本章前面给出的三层护栏，就是这个母题在安全上的实例。后文再遇到它们时，本书只标注模式名与本章的差异，不再重新推导。

## 发现循环：证据、提案、实验、反馈

上一节的五个模式是局部结构。还有一个更大的结构横跨三章，但因为它被拆开建，很容易被当成三条各自独立的管线。

第七章要从失败轨迹里定位首个错误、判定错误类别；第三章要把一条新证据变成一个尽可能小、有据可依的知识变更；第九章要判断一次修改是否真的让系统变好，然后决定发布还是回滚。这三件事用的机制完全不同——归因靠的是 Rubric 与首错定位，提案靠的是提议者—审核者，验证靠的是边界集 + 保留集与灰度回滚——所以它们**不是同一套机制的三次重复**，把它们的词汇强行统一反而会掩盖真正的差别。

真正共同的是它们在同一个循环里各占一段：

```text
证据（第七章）：失败轨迹 → 首个错误 + 错误类别
  → 提案（第三章）：证据 → 最小、可审查、可回滚的一次修改
  → 实验（第九章）：在边界集与保留集上实测，灰度发布
  → 反馈：实测结果与新的失败轨迹回到证据段
```

这个循环最近被 Jeff Dean 等人创办的 Discovery Loop 命名并推向自动化：提出实验、实现它、评估它、拿到结果，再把结果喂给下一轮，把原本串行的研究过程并行化[^ch1-discovery-loop]。需要说明的是，该公司成立于 2026 年 8 月，目前只公布了使命，尚无公开的技术结果；本书引用它是取其**对循环的命名**，而不是把它当作证据——这正是第七章将反复强调的区分。

把 Agent 系统的这个循环和纯研究循环放在一起看，会发现前者多了两条约束，而它们恰恰是本书后面大量篇幅在处理的东西。**第一，实验必须扎根于真实观测。** 研究循环里的"实验"可以是一次训练跑分，Agent 系统里的"实验"却在改动一个正在服务用户的系统，因此判定必须来自环境的真实状态（测试是否通过、数据库最终状态、工具返回），而不是模型对自己行为的复述。**第二，每次实验都要同时回答"它改好了什么"和"它弄坏了什么"。** 研究循环通常只追求指标上升，Agent 系统还必须证明没有破坏原本正确的行为——这就是上一节"边界集 + 保留集"存在的理由。

后面三章各自只讲自己那一段，不再重述整个循环：第三章讲提案怎样才算有据可依，第七章讲证据怎样才算可信，第九章讲实验与反馈怎样才能长期运转而不发散。

[^ch1-discovery-loop]: Discovery Loop 由 Jeff Dean、Sanjay Ghemawat、Quoc Le 和 Oriol Vinyals 于 2026 年 8 月 5 日宣布创立，是一家以"自动化机器学习、科学与工程"为使命的公益公司，其公开表述是自动化完整的实验循环、把原本串行的实验大规模并行化。参见 https://techcrunch.com/2026/08/05/jeff-dean-and-other-top-ai-researchers-are-leaving-google-to-launch-their-own-startup/ 。截至本书写作时，该公司尚未发表可复现的技术结果。
...
**五个模式贯穿全书**：提议者—审核者、渐进式披露、只增不改、边界集 + 保留集、最小 diff + 可回滚——它们共享同一个母题，即把判断从模型自己挪到模型之外的机制上。后续各章按名调用，不再重新推导。

**发现循环横跨三章**：证据（第七章的失败归因）→ 提案（第三章的知识更新）→ 实验与反馈（第九章的验证与发布）。三段用的机制本就不同，共同的不是词汇而是位置；相比纯研究循环，Agent 系统的这个循环还多两条约束——实验必须扎根真实观测，且每次都要同时回答"改好了什么"和"弄坏了什么"。

**安全是架构问题**：护栏、人工干预、对齐（alignment，即让模型的行为与人类意图保持一致）——安全问题从第一行代码就要考虑，而不是上线前打补丁。护栏按被绕过的难度分为上下文层、执行层与数据层三层，后续各章的安全讨论都挂在这个骨架上。
