...
Эксперимент 2-8 — небольшая качественная демонстрация, дающая интуитивное представление. Чтобы количественно оценить пользу и границы подхода «заранее посчитать и сразу посмотреть», автор и соавторы использовали специальный бенчмарк[^ch2-8] (единое название подхода — **дистилляция контекста, Context Distillation**; строка состояния агента — его повседневная форма). Выводы:

- **Заранее рассчитанная строка состояния** возвращает **слабым моделям точность**. Точность самых слабых моделей выросла на 40–54 процентных пункта, а локальная модель 2B на этих задачах даже сравнялась с передовой моделью без строки состояния.
- **Сильные модели и так отвечают правильно; их выигрыш — эффективность.** Та же строка состояния примерно на порядок сокращает объём рассуждений, задержку и стоимость каждого запроса (число токенов рассуждения уменьшается на 80–90% и более).
- Главное изменение таково: без строки состояния объём рассуждений на запрос **постоянно растёт** вместе с контекстом, а с ней становится **практически постоянным**. Какой бы длины ни был контекст, модели достаточно «взглянуть» на несколько ячеек состояния.


Однако между правильным и неправильным предварительным расчётом огромная разница. Три урока:
...
**Во-вторых, не удаляйте исходный контекст.** Строка состояния — **проекция с потерями** исходного контекста: она заранее вычисляет только те измерения, о которых, как вы ожидаете, спросят. Если её достаточно для подсчёта или отслеживания состояния, исходные записи можно удалить и сэкономить много токенов; но если вопрос относится к неучтённому измерению, точность одной лишь строки состояния резко обрушится.

**В-третьих, контролируйте точность строки состояния как первостепенный production-показатель.** Эксперимент показал, что **модель почти безоговорочно верит строке состояния**: если там написано «звонили 3 раза», она принимает это за три раза без проверки и пересчёта. Именно поэтому строка состояния эффективна, но любая ошибка в ней **без изменений** переходит в итоговый ответ. Поэтому упомянутый ранее риск **отравления строки состояния** требует серьёзного внимания.
