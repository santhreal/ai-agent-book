...
>
> **Техническое решение**: использовать реализацию проекта `PermissionEmbeddedDataObjects`, предоставляющую промежуточный слой Python-хранилища объектов поверх PostgreSQL. Типы данных объявляют правила разрешений, контекст доступа, валидаторы, связи объектов и реакции. Каждое чтение и запись проходят проверки разрешений и валидацию, сохранение и контроль ссылочной целостности, а затем управляемые асинхронные реакции. Сначала запускается детерминированная демонстрация конвейера найма без LLM; при желании модели генерируют атакующие операции против сырого SQL и PEDO API, после чего сравниваются состояния базы. Важен не вопрос, содержит ли сгенерированный обработчик правильный `if`, а то, принимается или отклоняется один и тот же запрос при достижении стабильного уровня данных.
>
> **Критерии приёмки**: корректное обновление конвейера найма выполняется; уровень данных отклоняет пропуск перехода состояния кандидата, зарплату вне диапазона должности и чтение между арендаторами. Должны пройти тесты разрешений, валидации, изоляции арендаторов, ссылочной целостности и реакций. Реализация включена в сопутствующий проект главы 5 `permission-embedded-data-objects`.
...
На этом мы завершили часть книги, посвящённую «построению агента», а генерация кода как раз является самой универсальной из рассмотренных мета-способностей. Но остался нерешённым один ключевой вопрос: как научно измерить эффективность всех этих проектных решений? Начиная со следующей главы книга переходит к теме «оценки и эволюции»: глава 7 строит методологию, охватывающую среду оценки, датасеты, автоматизированное судейство и выбор моделей, а главы 8 и 8 рассматривают непрерывное совершенствование соответственно на уровне параметров и на уровне всей системы агента.
