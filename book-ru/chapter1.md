...
Девять последующих глав снова и снова обращаются к одному и тому же набору структур. Они не принадлежат какой-то одной главе — это повторяющиеся решения под одним и тем же ограничением, — поэтому здесь мы называем их разом и даём каждому каноническое определение. Дальше главы вызывают их по имени и описывают лишь местный вариант.
...
**Минимальный diff и обратимость (Minimal Diff, Reversible)**: каждое изменение по возможности небольшое, с указанием источника и с возможностью отката по отдельности, а не сплошная переписка. Именно это делает возможной атрибуцию: когда что-то ломается, можно проследить до конкретного изменения. Обновления знаний главы 3, патчи кода главы 5, обновления промптов и программ главы 9 следуют этому правилу; а три пути обновления, приведённые в начале этой главы (адаптация внутри контекста, обновление внешних артефактов, обновление параметров), как раз выстроены по убыванию обратимости.

Эти пять шаблонов объединяет один мотив: **перенести суждение с «решает сама модель» на «решает механизм вне модели»** — проверяющий вне контекста, каталог вне основного текста, кэш вне изменения, удерживающий набор вне граничного, откат вне коммита. Трёхслойные ограждения, приведённые ранее в этой главе, — тот же мотив в применении к безопасности. Встречая эти шаблоны дальше, книга называет их и указывает местное отличие, а не выводит заново.

## Цикл открытия: свидетельство, предложение, эксперимент, обратная связь

Пять шаблонов предыдущего раздела — локальные структуры. Есть и более крупная структура, охватывающая три главы, но, поскольку её строят по частям, её легко принять за три независимых конвейера.

Главе 7 нужно найти в неудачной траектории первую ошибку и определить её класс. Главе 3 нужно превратить одно новое свидетельство в как можно меньшее и обоснованное изменение базы знаний. Главе 9 нужно решить, действительно ли правка сделала систему лучше, а затем — выпускать её или откатывать. Эти три дела опираются на совершенно разные механизмы: атрибуция — на рубрики и локализацию первой ошибки, предложение — на схему «Предлагающий—Проверяющий», проверка — на граничный и удерживающий наборы с канареечным выпуском и откатом. Поэтому они **не являются троекратным повторением одного механизма**, и насильственная унификация их словаря скрыла бы как раз существенные различия.

По-настоящему общим у них является положение: каждое занимает свой отрезок одного и того же цикла.

```text
Свидетельство (глава 7): неудачная траектория → первая ошибка + класс ошибки
  → Предложение (глава 3): свидетельство → одно минимальное, проверяемое, обратимое изменение
  → Эксперимент (глава 9): измерение на граничном и удерживающем наборах, канареечный выпуск
  → Обратная связь: измеренные результаты и новые неудачные траектории возвращаются к отрезку свидетельства
```

Недавно этот цикл получил имя и был двинут в сторону автоматизации компанией Discovery Loop, основанной Джеффом Дином с коллегами: предложить эксперимент, реализовать нужное для него, оценить его, передать результат в следующий круг и распараллелить процесс, который прежде шёл последовательно[^ch1-discovery-loop]. Следует сказать прямо: компания основана в августе 2026 года и пока опубликовала только свою миссию, без публичных технических результатов; книга ссылается на неё ради **данного ею имени цикла**, а не как на свидетельство — именно то различие, на котором будет настаивать глава 7.

Если поставить этот цикл агентной системы рядом с чисто исследовательским, обнаружатся два дополнительных ограничения, и как раз им посвящена значительная часть оставшейся книги. **Первое: эксперимент должен опираться на реальное наблюдение.** В исследовательском цикле «эксперимент» может быть прогоном обучения; в агентной системе он меняет систему, которая прямо сейчас обслуживает пользователей, поэтому вердикт должен исходить из фактического состояния среды — проходят ли тесты, каково итоговое состояние базы данных, что вернул инструмент, — а не из пересказа моделью собственных действий. **Второе: каждый эксперимент обязан ответить сразу и на «что он починил», и на «что он сломал».** Исследовательскому циклу обычно достаточно роста метрики; агентная система должна ещё показать, что не испортила поведение, которое и так было правильным. Именно ради этого в предыдущем разделе существуют граничный и удерживающий наборы.

Три последующие главы разбирают каждая только свой отрезок и не пересказывают цикл целиком: глава 3 — что делает предложение обоснованным, глава 7 — что делает свидетельство надёжным, глава 9 — что позволяет эксперименту и обратной связи работать долго и не расходиться.

[^ch1-discovery-loop]: Discovery Loop была объявлена 5 августа 2026 года Джеффом Дином, Санджаем Гемаватом, Куоком Ле и Ориолом Виньялсом как public benefit corporation с миссией автоматизировать машинное обучение, науку и инженерию; публично она описывает себя как автоматизацию полных экспериментальных циклов и масштабное распараллеливание того, что прежде выполнялось последовательно. См. https://techcrunch.com/2026/08/05/jeff-dean-and-other-top-ai-researchers-are-leaving-google-to-launch-their-own-startup/ . На момент написания книги воспроизводимых технических результатов не опубликовано.
...
**Пять шаблонов проходят через всю книгу**: Предлагающий—Проверяющий, постепенное раскрытие, только добавление, граничный набор + удерживающий набор и минимальный обратимый diff — все они объединены одним мотивом: перенести суждение от самой модели к механизму вне её. Дальнейшие главы вызывают их по имени, а не выводят заново.

**Цикл открытия охватывает три главы**: свидетельство (атрибуция отказов в главе 7) → предложение (обновление знаний в главе 3) → эксперимент и обратная связь (проверка и выпуск в главе 9). Три отрезка опираются на действительно разные механизмы; общее у них — положение, а не словарь. В отличие от чисто исследовательского цикла, цикл агентной системы несёт два дополнительных ограничения: эксперимент должен опираться на реальное наблюдение, и каждый круг обязан ответить и на то, что он починил, и на то, что он сломал.

**Безопасность — это архитектурный вопрос**: ограждения, вмешательство человека, согласованность (alignment — приведение поведения модели в соответствие с намерениями человека) — вопросы безопасности нужно продумывать с первой строки кода, а не латать перед запуском. Ограждения делятся на три слоя — контекста, исполнения и данных — по трудности обхода, и все дальнейшие главы вешают свои рассуждения о безопасности на этот каркас.
