...
Равноправное сотрудничество обычно предполагает 2-3 агентов равного статуса, которые через несколько итераций дают друг другу обратную связь. Основная ценность этого подхода — в привнесении когнитивного разнообразия: разные агенты рассматривают одну и ту же проблему с разных сторон, находят баланс между новаторством и надёжностью и в итоге выдают результат более высокого качества, чем мог бы дать любой отдельный агент.
...
**Парадигма предложитель-рецензент.**
...
>
> **Предварительные требования**: в этом эксперименте совместно используются технологии Computer Use и голосового агента из девятой главы, рекомендуется сначала выполнить соответствующие эксперименты из девятой главы.
>
...
>
> **Предварительные требования**: рекомендуется сначала ознакомиться с механизмом событийного управления и прерываний из четвёртой главы.
>
...
Конфликт параллельного доступа — это проблема на уровне файлов, с которой справляется опыт операционных систем; а каскадное усиление ошибок возникает там, где аналогия с процессами перестаёт работать: процессы передают друг другу байты с побитовой точностью, а агенты передают семантику, и каждый пересказ — это перекодирование с потерями. Когда несколько агентов часто взаимодействуют друг с другом, ошибка одного агента может усиливаться каждым последующим агентом, как в игре в «испорченный телефон», где сообщение искажается всё сильнее с каждой передачей.
...
Агент перевода A: переводит вторую главу, читает из глоссария,
        переводит "reasoning tokens" как "推理 token"
Агент перевода B: переводит седьмую главу, тоже переводит "inference latency" как "推理延迟"
        ↓ записывают перевод каждой главы
Агент проверки: видит, что во всей книге единообразно используется "推理",
        считает, что терминология согласована и перевод верен ✗
```

В чём проблема? "reasoning" (процесс размышления модели) и "inference" (прямой проход модели / развёртывание и работа модели) — два разных понятия, но поскольку терминологический агент изначально перевёл reasoning как "推理", последующие агенты, встретив inference, естественным образом выбрали то же самое слово — два разных понятия слились в один перевод, и читатель уже не сможет их различить. Правильным было бы перевести reasoning как "思考" (мышление), а inference как "推理" (вывод). Но агент проверки, увидев, что вся книга "единообразно" использует "推理", решил, что качество перевода высокое.

Одна терминологическая ошибка, пройдя через трёх агентов, обрела более высокую степень доверия именно благодаря "согласованности". Именно поэтому в этой книге принята переводческая конвенция reasoning = «размышление», inference = «вывод» (об этом сказано во введении): разные русские слова устраняют неоднозначность. Стоит подчеркнуть: «ошибка» здесь не обязательно является галлюцинацией — в приведённом примере источником была именно неверная терминологическая решение, но оно всё равно было усилено многократной "согласованностью"; но если бы источником действительно была галлюцинация (например, в эксперименте 10-2 агент-переводчик из-за рассеянного внимания «вспомнил» несуществующее терминологическое правило), механизм усиления был бы совершенно тем же, только последствия оказались бы серьёзнее. Эта цепочка усиления ошибок особенно опасна в модели оркестрации — если менеджер принял решение о распределении задач на основе ошибочного резюме от какого-то из подчинённых агентов, вся последующая работа остальных подчинённых агентов может строиться на неверной предпосылке.

**Перекрёстная проверка** — ключевой способ разорвать эту цепочку. Суть не в том, чтобы подключить к одной и той же цепочке рассуждений больше агентов, а в том, чтобы какой-то агент пересмотрел вывод с **независимой точки зрения**: не глядя на процесс размышлений предыдущих агентов, а глядя только на исходные свидетельства и на то, соответствует ли им итоговый вывод. Это как раз развитие механизма предложитель-рецензент, обсуждавшегося в пятой главе, применительно к мультиагентному сценарию: ценность Reviewer заключается не только в обнаружении ошибок в коде или проблем форматирования, но и в том, что как независимый судья он способен выявить противоречие во всей цепочке рассуждений, которое было коллективно упущено. Для решений с высоким риском можно также подключать внешние средства проверки — например, обратную связь от детерминированных инструментов вроде юнит-тестов, компиляторов, запросов к базам данных, которая не подвержена галлюцинациям и является самым надёжным «разрывателем цепи».

У преждевременного завершения есть симметричная противоположность: **неконтролируемый цикл**. В предыдущем разделе «равноправное сотрудничество» речь шла о ситуации «должен был зациклиться, но не зациклился» — агент бросает работу на полпути; здесь же нужно остерегаться обратного: «цикл крутится не переставая, но становится всё хуже». В практике Loop-инженерии индустрия выделила три типичных провальных паттерна: во-первых, **неконтролируемые затраты токенов** — цикл работает без присмотра часами, сжигая огромный бюджет и производя горы кода, который никто не заказывал; во-вторых, **долг понимания** (comprehension debt) — чем быстрее цикл поставляет код, тем сильнее отстаёт понимание инженером того, как система на самом деле устроена, и когда вмешательство человека становится необходимым, он уже не понимает собственную систему; в-третьих, **когнитивная капитуляция** (cognitive surrender) — проектировщик привыкает, что цикл делает всё за него, постепенно отказывается от самостоятельного мышления и проверки, и качество спирально падает. Лекарство от всех трёх проблем то же самое, что и для разрыва цепочки усиления ошибок: явный бюджет и условия завершения, верификаторы, опирающиеся на реальные наблюдения, а также то, что человек всегда остаётся «инженером цикла», а не просто «тем, кто нажал кнопку старта».

Всё вышеизложенное обсуждалось с инженерной точки зрения — как заставить группу агентов совместно выполнить задачу. Далее взгляд смещается: что возникает, когда большое число агентов сосуществует долгое время и уже не движимо единой целью? Этот раздел относится к области передовых исследований, и инженерно-ориентированные читатели могут читать его выборочно.

### Режим отказа третий: преждевременное завершение и неконтролируемые циклы
...
### Режим отказа четвёртый: долг понимания и когнитивная капитуляция
