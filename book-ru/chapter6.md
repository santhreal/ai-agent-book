...
Обучающий корпус модели почти целиком пошаговый: за вопросом следует ответ, за вызовом инструмента — его результат, один договорил, и только тогда открывает рот другой. Поэтому выученная моделью стратегия исходит из того, что мир её подождёт. Реальная среда не ждёт: письмо приходит, пока она думает; пользователь вклинивается на середине фразы; страница уже изменилась между двумя скриншотами; чашку опрокинули, пока манипулятор к ней тянулся. **Четыре раздела этой главы — это то самое допущение, ослабляемое по одному на четырёх временных масштабах.**

Сначала посмотрим, где они расположены:
...
А начинается глава с **асинхронности и событийности** потому, что это ближе всего читателю: модальность по-прежнему чисто текстовая, меняется только момент. Это первый шаг за пределы пошагового мира предыдущих пяти глав и первое место, где тезис о «допущении пошаговости» касается земли.

## Асинхронность и событийность: когда мир приходит сам

Три категории инструментов из главы 4 — восприятия, выполнения и сотрудничества — агент вызывает сам: он решает, когда посмотреть и когда действовать. Этот раздел переходит к самому медленному концу оси момента: как агент управляет задачами длиной в часы и даже дни и как отвечает на внешние события, способные прийти в любую минуту? Для этого нужна событийно-ориентированная асинхронная архитектура; а оставшиеся две из пяти категорий инструментов главы 1 — инструменты срабатывания событий и коммуникации с пользователем — работают как раз опираясь на эту архитектуру, поэтому рассматриваются здесь же.
...
[^ch6-async-1]: О том, как около двухсот строк оркестрации превращают уже существующую модель-мыслитель в агента с непрерывным размышлением, а также о выводе «обучающий сигнал определяет, полезно ли непрерывное размышление», см. Li, Bojie and Noah Shi. *Never Stop Thinking: Continuous-Time Language Agents.* 2026 (готовится к публикации).
...
[^ch6-13]: Полное измерение того, когда меняется преимущество каскада и сквозной модели, см. Li, Bojie и Noah Shi, *The Cascade Gap: When and Why Self-Cascades Help Multimodal Agents*, 2026 (готовится к публикации).
...
> Таблица 6-1. Локальные результаты MiniCPM-o 4.5: сквозной режим и самокаскад (четыре механические проверки, а не бенчмарк)
>
>
...
>
> Выборка мала и не доказывает, какой путь обычно точнее или быстрее. Версии, сырые ответы и аудио-в-аудио свидетельства находятся в [chapter6/end-to-end-speech](../chapter6/end-to-end-speech/).
...
#### Решение 3: сквозное объединение мышления и выражения (на примере Step-Audio R1)
...
В идеале модель должна определять эмоцию по высоте тона, ритму и интонации, а не только по расшифрованному тексту. Так называемое «мышление через текстовый суррогат» — это когда модель подменяет анализ мелодии и акустических признаков негативными словами в тексте песни. MGRD отбирает те цепочки рассуждений, которые действительно опираются на акустические признаки, обучает на этих данных модель и с помощью обучения с подкреплением не даёт модели пропустить рассуждение и сразу угадать ответ.

В MPS мозг замысла непрерывно порождает фрагменты рассуждения, а мозг выражения, получив фрагмент, тут же синтезирует речь, соединяя его с уже произнесённым. Оба работают параллельно, конвейером, поэтому не нужно ждать окончания всего рассуждения, чтобы пользователь услышал первую фразу (рис. 6-11).


![Рис. 6-11: Двухмозговая архитектура MGRD и MPS в Step-Audio R1](images/fig6-11.svg)


Единая модель теснее всего реализует «думать во время речи», но ценой того, что мышление и речь в реальном времени приходится переобучать вместе; в развязанном варианте фоновый «мозг» проще заменить, а единый лучше подходит специализированным сценариям, где важна предельная естественность. Это компромисс, а не простая замена одного другим.
...
![Рис. 6-12 Цикл восприятие-мышление-действие Computer Use-агента](images/fig6-12.svg)
...
Anthropic определяет три категории инструментов, составляющих полную способность к взаимодействию (Рис. 6-13):


![Рис. 6-13 Пространство действий Computer Use](images/fig6-13.svg)
...
>
> Путь B использует сопроводительный проект книги [`chapter6/computer-use-open-model`](../chapter6/computer-use-open-model/). По умолчанию он управляет browser-use с помощью модели с открытыми весами Qwen3-VL 32B Instruct—через размещённый API OpenRouter либо посредством перенаправления `OPEN_MODEL_BASE_URL` на самостоятельно размещённый vLLM/SGLang или другую совместимую конечную точку. Конечная точка должна принимать скриншоты и поддерживать нативный JSON Schema; если поддерживается только обычный JSON, можно явно включить режим совместимости schema-in-prompt.
>
...
![Рис. 6-14 Set-of-Mark и структурированное индексирование элементов (реализация browser-use)](images/fig6-14.svg)
...
[^ch6-9]: Полное описание механизма трёх компонентов — шлюзование ключевых кадров, транскрипция по необходимости, превращение кадров в устойчивый текст — и абляции по каждой модели см. Li, Bojie and Noah Shi. *Agent-Computer Observation Interfaces Enable Dynamic Computer Use.* arXiv:2606.29472, 2026.
...
Короткие отрезки реагируют быстрее, но умножают число вызовов модели; длинные плавнее, но склонны опираться на устаревшее наблюдение. Эксперимент 6-12 сравнивает такие компромиссы в симуляторе, а границы безопасности настоящего железа касается эксперимент 6-11.
...
Эта глава сняла предпосылку, которую предыдущие пять принимали по умолчанию: что агент и мир говорят по очереди.

Если читать вдоль осей **модальности** и **момента**, четыре раздела оказываются одним тезисом, развёрнутым на четырёх временных масштабах. **Асинхронность и событийность** расширяют наблюдение от «агент сам идёт за ним» до «мир его проталкивает», а действие — от «завершить внутри хода» до «сначала запустить, потом завершить по событию»; модальность не меняется, меняется лишь момент. **Голос** сжимает масштаб до миллисекунд, и главная линия эволюции трёх парадигм — каскада, сквозной Omni и полного дуплекса — это как раз переход от «говорить по очереди» к непрерывному слушанию и говорению с разделением труда между интерактивностью переднего плана и глубоким размышлением фона. **Computer Use** переносит тот же замкнутый цикл на экран, где узкое место расширилось от «удастся ли выполнить задачу» до эффективности операций, непрерывного визуального понимания и подтверждения состояния после действия. **Робототехника** доводит его до физического мира, где разбиение действий балансирует между плавностью и скоростью реакции, а вопрос, выполнено ли задание, по-прежнему должно решать новое наблюдение.
...
Они делят и один набор примитивов — пробуждение, безопасная точка, отмена, вытеснение, разделение быстрого и медленного. «Проверить сигнал отмены в безопасной точке» в цикле событий и «при аномалии отбросить оставшиеся действия и наблюдать заново» в разбиении действий — это один механизм, реализованный дважды на масштабах, отличающихся на пять порядков; разделение труда между быстрой моделью переднего плана и медленной фоновой и «сначала вернуть идентификатор задачи, а завершит её позднее событие» у асинхронного агента — тоже две записи одного и того же.

Стоит отметить, что большинство этих механизмов пока остаются инженерными полумерами. Протоколы-заглушки, событийные метки в строке состояния, отмена и откат упреждающего размышления — по сути все они компенсируют оркестрационной логикой асинхронный опыт, которого нет в обучающем распределении модели. Прогресс на стороне модели уже втягивает часть этого в параметры: интерактивные модели встроили прерывание и вклинивание внутрь модели, а непрерывное размышление позволило «думать, слушая» не дожидаясь следующего поколения. Но пока обучающие корпуса остаются преимущественно пошаговыми, этот компенсирующий слой не исчезнет — он лишь будет перемещаться к новой границе вслед за ростом возможностей модели.

Эта глава завершает последний фрагмент части «как строить агента»: пространства наблюдений и действий развёрнуты уже по всем трём направлениям — содержанию, модальности и моменту. Следующие три главы поворачивают к другому вопросу: как узнать, что всё это построено верно, и как сделать так, чтобы оно продолжало улучшаться.
