...
Интуиция прежняя — чем реже термин, тем больше его вес, — меняется лишь способ измерения. В числителе вместо общего числа документов $N$ стоит число документов, *не содержащих* термин, $N - \text{DF}(t)$, поэтому отношение прямо показывает, во сколько раз документов без термина больше, чем с ним. Прибавление 0.5 к числителю и знаменателю сглаживает результат, и формула остаётся определённой в обоих крайних случаях — при $\text{DF}(t) = 0$ и при $\text{DF}(t) = N$. Плата за это — отрицательный вес для термина, встречающегося более чем в половине документов ($\text{DF}(t) > N/2$), поэтому в реализациях его обычно ограничивают снизу. Этот вариант происходит из вероятностной модели поиска и известен в литературе как вес Robertson–Spärck Jones.
...
Второй этап — **слияние результатов** в единый пул. Оценки напрямую несопоставимы: косинусное сходство плотного поиска и оценка BM25 различаются по масштабу и распределению. Обычно либо нормализуют оценки каждого пути и берут взвешенную сумму, либо применяют Reciprocal Rank Fusion (RRF), отбрасывая исходные оценки и используя только ранги: оценка = Σ 1/(k + rank), где k — константа сглаживания, обычно 60. RRF прост и устойчив, но теряет богатый сигнал релевантности исходных оценок.
...
**Кейс второй: проблема границ в праве на скидку Xfinity**. На этот раз база знаний — архив обращений в поддержку: несколько сотен тикетов, каждый из которых фиксирует один реальный исход. Ветерану John одобрили заявку, врач Sarah получила скидку, учителю Mike сказали, что он не подходит под условия, и так далее. Каждый тикет содержит вывод по одному отдельному случаю; ни один не описывает сам объём права на скидку. Когда медсестра спрашивает «а мне скидка положена?», препятствия накладываются одно на другое. Во-первых, **смещение к ближайшему соседу**: «медсестра» семантически ближе всего к «врачу», поэтому тикет Sarah оказывается первым, и модель послушно заключает, что медсёстрам скидка тоже положена; окажись выше тикет Mike, тот же вопрос получил бы противоположный ответ. **Ответ определяется тем, какой тикет ближе к запросу, а не самой политикой.** Во-вторых, **отсутствие граничной семантики** — препятствие, которое не лечится увеличением k: формулировка вида «только ..., все прочие профессии не подходят» несёт квантор всеобщности и отрицание и не содержится ни в одном отдельном тикете, а существует лишь в замыкании всего корпуса. Архив в принципе не отвечает на вопрос «считается ли медсестра», поэтому если заставить модель вывести всеобщее правило из горстки отдельных случаев, такой вывод не был обоснован изначально. В-третьих, **отсутствие сигнала полноты**: модель никак не может понять, увидела ли она правило целиком, поэтому не переспрашивает, а уверенно отвечает по тем немногим тикетам, что оказались под рукой. Решение снова относится к этапу индексации: офлайн прочитать весь архив тикетов и, опираясь на официальную политику предоставления скидки как на источник истины (а не экстраполируя по нескольким найденным случаям — это как раз то загрязнение знаний, о котором сказано дальше), извлечь одну карточку правила: «Скидка Xfinity действует для военнослужащих действительной службы и ветеранов, а также для лицензированных медицинских работников, включая медсестёр; другие профессии, например учителя, не подходят; профессии, не указанные в списке, требуют проверки человеком». Когда и граница, и запасной вариант выписаны явно, один запрос даёт полное правило независимо от того, о какой профессии спрашивают: модели больше не нужно ничего выводить — достаточно сопоставить.
