...
Нативная обработка имеет самый высокий потолок возможностей: кодировщики вроде Vision Transformer отображают разные данные в общее семантическое пространство. Извлечение текста подходит моделям без нативной поддержки и экономнее для текстовых PDF, но теряет макет, графики и изображения. Если основная модель не мультимодальна, инструменты `analyze_image`, `analyze_pdf` и `analyze_audio` могут передать файл и вопрос специализированной модели и вернуть краткий ответ, не занимая контекст большими мультимодальными данными.
