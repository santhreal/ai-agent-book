...
### Метрики процесса, безопасности и устойчивости

Конечного результата недостаточно. Доля валидных и разрешённых действий, смысловая правильность аргументов инструментов, эффективность пути (шаги, повторы, откаты), полнота поиска, стоимость и задержка показывают место сбоя. Для чувствительных операций, утечки данных и запрещённого содержимого действует **нулевая терпимость**. Устойчивость охватывает seed, изменения UI, нестабильность API и помехи от устаревшей памяти; необходимо проверять и **траекторию**, и фактический **итог** состояния системы.
...
### Атрибуция отказов и регрессии по префиксу траектории
