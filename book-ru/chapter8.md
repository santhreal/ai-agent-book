...
Если сжать это до минимального обучающего скелета, суть не в API конкретного фреймворка, а в границе: **токены промпта не дают супервизии, токены ответа — дают**.
...
Здесь `-100` лишь помечает маску потерь, а не убирает промпт из входа модели: модель всё равно должна прочитать вопрос, чтобы выучить протокол ответа.
...
Тот же механизм можно записать python-подобным скелетом ниже. В нём опущены параллелизм сэмплирования, KL-регуляризация и детали оптимизатора — показана только причинная цепочка от одного rollout до обновления параметров:
...
Эту границу можно записать совсем короткой маской уровня траектории:
...
Это не значит, что обратная связь среды не важна: именно она обеспечивает расчёт награды и преимущества, просто её не следует принимать за целевую последовательность, которую политике надлежит воспроизводить. Сообщения инструментов обязаны сохранять пометку источника и оставаться отличимыми от вывода политики внутри среды исполнения.
...
On-Policy Distillation была систематически сформулирована и популяризована Thinking Machines Lab в 2025 году[^ch8-10]. Она берётся исправить слабости SFT и RL сразу: супервизия SFT плотная, но приходит с **off-policy путей**, пройденных учителем или человеком; когда ученик ошибается сам и попадает в состояния, не покрытые обучающими данными, он не знает, как выправиться. RL позволяет ученику порождать собственные **on-policy пути**, но у траектории обычно есть лишь одна финальная награда, так что сигнал разрежен и обладает большой дисперсией.
...
Поток управления сжимается так: ученик сам идёт своим путём, а учитель даёт распределение только в тех состояниях, которые ученик действительно посетил, и не проигрывает за него off-policy ответ.
...
Этот скелет нужен лишь для того, чтобы отличить on-policy состояния от потокенной супервизии.
...
OPSD можно прочесть как ограниченный вариант предыдущего скелета:
...
Таблица 8-4. Соответствие оценочных наборов главы 7 их применению в обучении в главе 8
