"""Regression: ContextualChunker must keep chunk steps at least 1."""
import sys
import types
from dataclasses import dataclass


def _stub():
    sys.modules.setdefault("requests", types.ModuleType("requests"))
    sys.modules.setdefault("openai", types.ModuleType("openai"))
    sys.modules["openai"].OpenAI = object
    cfg = types.ModuleType("config")

    @dataclass
    class ChunkingConfig:
        chunk_size: int = 1000
        chunk_overlap: int = 100
        max_chunk_size: int = 2000
        min_chunk_size: int = 1
        respect_paragraph_boundary: bool = True

    cfg.ChunkingConfig = ChunkingConfig
    cfg.KnowledgeBaseConfig = object
    cfg.KnowledgeBaseType = object
    cfg.LLMConfig = object
    sys.modules["config"] = cfg


_stub()

from contextual_chunking import ContextualChunker  # noqa: E402
from config import ChunkingConfig  # noqa: E402


def test_chunk_size_zero_long_unsplittable_sentence():
    cfg = ChunkingConfig(chunk_size=0, max_chunk_size=40, min_chunk_size=1)
    chunker = ContextualChunker(chunking_config=cfg, use_contextual=False)
    text = "alpha" * 80
    chunks = chunker._chunk_by_paragraphs(text, "d1")
    assert isinstance(chunks, list)
    assert len(chunks) >= 1


def test_chunk_size_equals_overlap_size_based():
    cfg = ChunkingConfig(
        chunk_size=50,
        chunk_overlap=50,
        min_chunk_size=1,
        respect_paragraph_boundary=False,
    )
    chunker = ContextualChunker(chunking_config=cfg, use_contextual=False)
    chunks = chunker._chunk_by_size("hello world " * 20, "d2")
    assert isinstance(chunks, list)
    assert len(chunks) >= 1
    assert all(c["text"] for c in chunks)
