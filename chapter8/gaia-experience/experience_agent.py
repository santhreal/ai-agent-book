...
class ExperienceAgent(Agent):
    ...
    async def execute_task(self, task: Task) -> TaskResponse:
        ...
        if self.apply_experience:
            ...
            if relevant_experiences:
                ...
                logger.info(
                    f"Applied {len(relevant_experiences)} relevant experiences to prompt"
                )
        ...
        if (
            self.learning_mode
            and task_response
            and self._is_successful(task_response, task)
        ):
    ...
    def _extract_trajectory(
        self, task_response: Optional[TaskResponse]
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _learn_from_success(
        self, question: str, response: TaskResponse, trajectory: List[Dict[str, Any]]
    ):
        """
