...
def load_model(model_path: str, base_model: str = "Qwen/Qwen3-30B-A3B-Instruct-2507"):
    ...
    print("This may take a few minutes for the 30B model...")
...
def evaluate_model(
    ...
    for idx, sentence in enumerate(test_sentences):
        ...
        if ground_truth_labels and idx < len(ground_truth_labels):
            ...
            display_sentence = (
                sentence if len(sentence) <= 60 else sentence[:57] + "..."
            )

            print(
                f"{status_color} [{idx + 1:4d}/{len(test_sentences)}] "
...
def build_confusion_matrix(
    predictions: List[str], ground_truth: List[str], sentences: List[str]
) -> Dict:
    """
    ...
    # Build matrix
    for idx, (pred, gt, sentence) in enumerate(
        zip(predictions, ground_truth, sentences)
    ):
        if pred is None:
            pred = "None"
        ...
        if pred == gt:
            ...
            if (
                len(per_language_stats[gt]["errors"]) < 5
            ):  # Keep first 5 errors per language
    ...
    sorted_languages = sorted(language_accuracy.items(), key=lambda x: x[1]["accuracy"])
...
def print_confusion_matrix(confusion_data: Dict):
    ...
    for lang, stats in sorted_langs:
        ...
        print(
            f"{symbol} {lang:>2s}: {acc:5.1f}% ({stats['correct']:4d}/{stats['total']:4d})"
        )
    ...
    for idx, (lang, stats) in enumerate(worst_languages, 1):
        ...
        if stats["errors"]:
            ...
            for err in stats["errors"][:3]:  # Show first 3 errors
                ...
                sentence_preview = (
                    err["sentence"][:50] + "..."
                    if len(err["sentence"]) > 50
                    else err["sentence"]
                )
                print(
                    f"     - Predicted {pred_lang} (should be {lang}): {sentence_preview}"
                )
...
def main():
    ...
    if args.ground_truth_file:
        ...
        print(
            f"Matched {matched}/{len(test_sentences)} test sentences to training data"
        )

        if matched == 0:
            print(
                "⚠️  Warning: No ground truth matched. Results will not show accuracy."
            )
            ground_truth = None
    ...
    output = {
        ...
        "timestamp": str(Path(args.model_path).stat().st_mtime)
        ...
        "summary": {
            ...
            "parse_rate": results["predicted"] / results["total"]
            if results["total"] > 0
            else 0,
        },
