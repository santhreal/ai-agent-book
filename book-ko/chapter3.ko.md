...
마지막으로 다른 장의 내용과 혼동하지 않도록 경계를 분명히 해야 합니다. 이 절은 메모리 **저장 계층**의 정리 알고리즘, 즉 어떤 메모리를 선택·클러스터링·추상화하고 어떤 형태로 바꿀지를 다룹니다. 2장의 컨텍스트 압축은 단일 세션 안에서 창 크기 문제를 해결하므로 두 메커니즘은 서로 다른 계층에서 작동합니다. 이 장은 지식의 저장, 색인, 검색도 담당합니다. 9장은 “온라인으로 증거를 추가하고 오프라인으로 통합한다”라는 2단계 패턴을 에이전트 행동의 진화로 일반화하여, 어떤 운영 증거가 지속적 갱신을 촉발하기에 충분한지 살펴봅니다.
...
이 장과 앞 장은 모두 “컨텍스트” 문제를 다룹니다. 하나는 단일 세션 안에서, 다른 하나는 여러 세션에 걸쳐 다룹니다. 이 장이 주로 축적하는 것은 사용자와 세계에 관한 선언적 지식입니다. 9장은 동일한 추출 및 검색 인프라를 다시 사용하지만, 그 대상을 작업의 성공과 실패로 뒷받침되는 행동 지식, 즉 “어떤 조건에서 에이전트가 무엇을 해야 하는가?”로 바꿉니다. 다음 장에서는 “도구”를 다룹니다. 도구 설계, MCP 상호 운용 표준, 이벤트 기반 아키텍처를 포함하여 에이전트가 도구로 외부 세계와 상호작용하는 방법을 살펴봅니다.
