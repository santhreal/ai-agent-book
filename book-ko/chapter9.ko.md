...
>
> **데이터와 절차:** 정상 환불, 거짓 약속, 개인정보 공개, 과도한 거절이라는 네 범주를 포함하는 전문가 라벨 궤적을 준비합니다. 첫 번째 계층은 최종 주문 상태와 도구 로그를 읽어 환불이나 일정 변경이 실제로 이루어졌는지 판단합니다. 두 번째 계층은 각 단계를 비즈니스 정책과 대조하여 권한, 필수 절차, 개인정보 보호, 사실 근거, 약속-행동 일치를 확인합니다. 세 번째 계층은 표 9-1의 루브릭에 따라 언어 품질과 규정에 맞는 대안을 평가하고 각 실패의 증거로 관련 턴을 남깁니다. 기본 품질 판정자는 결정론적 규칙을 사용하며 실제 LLM 판정자도 사용할 수 있습니다. 위 계층에 어떤 모델을 사용하든 결과와 규칙 계층을 언어 모델의 추측에 맡겨서는 안 됩니다.
>
...
>
> 부록 구현은 [`trajectory-verifier`](../chapter9/trajectory-verifier/)에서 확인할 수 있습니다. 기본값은 오프라인에서 재현할 수 있는 품질 판정자를 사용하며, 구현된 실제 LLM 검증기를 실행하려면 `--judge llm`을 사용하십시오.
