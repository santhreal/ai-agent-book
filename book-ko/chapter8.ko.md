...
서두에서 세 단계의 지도를 제시했습니다. 이 절에서는 각 단계의 작동 원리를 자세히 살펴봅니다. 세 단계는 사용하는 **데이터**, **최적화 목표**, **비용**이 서로 다릅니다. 공통점과 차이점을 이해하는 것이 이 장 전체를 이해하는 열쇠입니다. 표 8-1에서 먼저 전체를 조망한 뒤 세부 내용을 설명합니다.

표 8-1 모델 역량을 만드는 세 단계
...
이 일을 최소한의 학습 뼈대로 압축하면, 핵심은 특정 학습 프레임워크의 API가 아니라 **프롬프트 토큰은 감독을 주지 않고 응답 토큰이 감독을 준다**는 경계에 있습니다.
...
여기서 `-100`은 손실 마스킹만을 뜻하며, 프롬프트를 모델 입력에서 지운다는 의미가 아닙니다. 모델은 응답 프로토콜을 배우기 위해 여전히 질문을 읽어야 합니다.
...
>
> 이 실험은 멀티모달 모델 학습의 기본 패러다임을 보여 줍니다. 단일 모달 사전 학습의 결과를 재사용하고 가벼운 투영 계층을 학습해 교차 모달 정렬을 달성합니다. 효율적이고 확장하기 쉽지만, 투영 계층의 제한된 표현력이 깊은 교차 모달 이해를 가로막을 수 있습니다. 같은 ‘비전 인코더 + 투영 계층 + LLM’ 아키텍처를 한 단계 확장해 모델이 행동을 출력하게 하면 6장에서 자세히 다룰 VLA(Vision-Language-Action) 모델이 됩니다.
...
같은 메커니즘은 아래의 Python 스타일 뼈대로 나타낼 수 있습니다. 샘플링 병렬화, KL 정규화, 옵티마이저 세부 사항은 생략하고 한 번의 rollout에서 파라미터 갱신까지의 인과 사슬만 표시했습니다.
...
이 경계는 아주 짧은 궤적 수준 마스크로 쓸 수 있습니다.
...
이는 환경 피드백이 중요하지 않다는 말이 아닙니다. 피드백은 보상과 이점 계산을 맡을 뿐, 정책이 재현해야 할 목표 시퀀스로 취급되어서는 안 된다는 뜻입니다. 도구 메시지는 출처 표시를 유지해야 하고 실행 환경 안에서 정책 출력과 구분되어야 합니다.
...
On-Policy Distillation(온폴리시 증류)은 2025년 Thinking Machines Lab이 체계적으로 제시하고 널리 알렸습니다[^ch8-10]. 이 방법은 SFT와 RL의 단점을 동시에 해결하려 합니다. SFT의 감독은 조밀하지만 교사나 사람이 걸어간 **오프폴리시 경로**에서 옵니다. 학생이 스스로 실수해 학습 데이터가 덮지 않은 상태로 들어가면 어떻게 회복해야 할지 모릅니다. RL은 학생이 스스로 **온폴리시 경로**를 만들게 하지만, 궤적 하나에 보통 최종 보상 하나뿐이라 학습 신호가 희소하고 분산이 큽니다.
...
그 제어 흐름은 이렇게 압축할 수 있습니다. 학생이 먼저 자기 길을 걷고, 교사는 학생이 실제로 방문한 상태에서만 분포를 제공하며, 학생 대신 오프폴리시 답안을 재생하지는 않습니다.
...
여기의 뼈대는 온폴리시 상태와 토큰 단위 감독을 구분하기 위한 것일 뿐입니다.
...
OPSD는 앞 문단 뼈대의 제한된 변형으로 볼 수 있습니다.
...
표 8-4 7장의 평가 데이터 세트에서 8장의 학습 용도로의 대응
