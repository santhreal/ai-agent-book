...
네이티브 처리는 Vision Transformer 같은 인코더로 서로 다른 데이터를 공통 의미 공간에 매핑하므로 능력의 상한이 가장 높습니다. 텍스트 추출은 네이티브 기능이 없는 모델과 텍스트 중심 PDF에서 토큰을 절약하지만 레이아웃·차트·이미지를 잃습니다. 주 모델이 멀티모달이 아니라면 `analyze_image`, `analyze_pdf`, `analyze_audio` 같은 도구로 파일과 질문을 전문 모델에 전달하고 짧은 결과만 컨텍스트에 남길 수 있습니다.
