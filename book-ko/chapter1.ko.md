...
유의할 점은 같은 메커니즘이 여러 층에 나타날 수 있다는 것입니다. 규칙 기반 정규식 필터는 입력도 출력도 거를 수 있습니다. 위 분류는 그것이 보장을 제공하는 위치를 따른 것입니다. 세 층은 대체 관계가 아니라 누적 관계입니다. 위층은 대부분의 공격을 바깥에서 막고 일찍 피드백을 주며, 아래층은 위층이 뚫렸을 때 침범할 수 없는 경계를 지킵니다.
...
뒤에 이어지는 아홉 장은 같은 구조들을 되풀이해 사용합니다. 그것들은 어느 한 장에 속하지 않고, 같은 제약 아래 반복해서 나오는 해법입니다. 그래서 여기서 한 번에 이름을 붙이고 규범적인 정의를 제시합니다. 이후의 각 장은 이름으로 호출하고, 그 장에서의 변형만 설명합니다.
...
**최소 diff + 롤백 가능(Minimal Diff, Reversible)**: 변경은 가능한 한 작게, 출처를 달고, 단독으로 되돌릴 수 있게 하며 통째로 다시 쓰지 않습니다. 이것이 귀인을 가능하게 합니다. 문제가 생겼을 때 어느 변경인지 짚어낼 수 있기 때문입니다. 3장의 지식 갱신, 5장의 코드 패치, 9장의 프롬프트와 프로그램 갱신이 모두 이를 따릅니다. 이 장 앞부분에서 제시한 세 가지 갱신 경로(컨텍스트 내 적응, 외부 산출물 갱신, 파라미터 갱신)도 되돌리기 쉬운 순서대로 배열된 것입니다.

이 다섯 패턴은 같은 모티프를 공유합니다. **판단을 "모델이 스스로 정한다"에서 "모델 바깥의 메커니즘이 정한다"로 옮기는 것**입니다. 검토자는 컨텍스트 바깥에, 목차는 본문 바깥에, 캐시는 변경 바깥에, 보존 집합은 경계 집합 바깥에, 롤백은 커밋 바깥에 있습니다. 이 장 앞에서 제시한 세 층 가드레일이 바로 이 모티프를 보안에 적용한 사례입니다. 뒤에서 이 패턴들을 다시 만날 때, 이 책은 패턴 이름과 그 장에서의 차이만 밝히고 다시 유도하지 않습니다.

## 발견 루프: 증거, 제안, 실험, 피드백

앞 절의 다섯 패턴은 국소적인 구조입니다. 세 장에 걸치는 더 큰 구조가 하나 더 있는데, 나뉘어 만들어졌기 때문에 서로 독립된 세 개의 파이프라인으로 오해되기 쉽습니다.

7장은 실패한 궤적에서 첫 오류를 짚어내고 오류 유형을 판정해야 합니다. 3장은 새 증거 하나를 가능한 한 작고 근거 있는 지식 변경으로 바꿔야 합니다. 9장은 어떤 수정이 정말로 시스템을 좋게 만들었는지 판단하고, 배포할지 되돌릴지를 정해야 합니다. 이 셋이 쓰는 메커니즘은 완전히 다릅니다. 귀인은 루브릭과 첫 오류 위치 파악에, 제안은 제안자-검토자에, 검증은 경계 집합과 보존 집합, 카나리 배포와 롤백에 기댑니다. 그러므로 이들은 **같은 메커니즘의 세 번 반복이 아닙니다**. 용어를 억지로 통일하면 오히려 정말 중요한 차이를 가리게 됩니다.

진짜로 공유하는 것은 위치입니다. 셋은 같은 루프에서 각각 한 구간을 차지합니다.

```text
증거(7장): 실패한 궤적 → 첫 오류 + 오류 유형
  → 제안(3장): 증거 → 최소하고 검토 가능하며 되돌릴 수 있는 하나의 수정
  → 실험(9장): 경계 집합과 보존 집합에서 실측, 카나리 배포
  → 피드백: 실측 결과와 새 실패 궤적이 증거 구간으로 돌아감
```

이 루프는 최근 Jeff Dean 등이 창업한 Discovery Loop가 이름 붙이고 자동화로 밀고 나간 것입니다. 실험을 제안하고, 필요한 것을 구현하고, 평가하고, 결과를 다음 회차로 보내며, 원래 직렬이던 과정을 병렬화한다는 것입니다[^ch1-discovery-loop]. 다만 분명히 밝혀 둘 것이 있습니다. 이 회사는 2026년 8월에 설립되었고 지금까지 사명만 공개했을 뿐 공개된 기술적 결과는 없습니다. 이 책이 이를 인용하는 것은 **루프에 이름을 붙였다는 점** 때문이지 증거로서가 아닙니다. 바로 7장이 거듭 강조할 구분입니다.

에이전트 시스템의 이 루프를 순수한 연구 루프와 나란히 놓으면 앞의 것에 두 가지 제약이 더 붙는다는 것을 알 수 있고, 그 두 가지가 바로 이 책의 뒷부분이 많은 지면을 들여 다루는 것입니다. **첫째, 실험은 진짜 관측에 뿌리내려야 합니다.** 연구 루프에서 "실험"은 한 번의 학습 실행일 수 있지만, 에이전트 시스템의 "실험"은 지금 사용자에게 서비스 중인 시스템을 바꿉니다. 그러므로 판정은 환경의 실제 상태—테스트 통과 여부, 데이터베이스의 최종 상태, 도구 반환값—에서 와야 하며, 모델이 자기 행동을 다시 진술한 것이어서는 안 됩니다. **둘째, 모든 실험은 "무엇을 고쳤는가"와 "무엇을 망가뜨렸는가"에 함께 답해야 합니다.** 연구 루프는 보통 지표 상승만 좇지만, 에이전트 시스템은 원래 옳던 동작을 깨뜨리지 않았음도 보여야 합니다. 앞 절의 경계 집합과 보존 집합이 존재하는 이유가 이것입니다.

뒤따르는 세 장은 각자 자기 구간만 다루고 루프 전체를 되풀이하지 않습니다. 3장은 제안이 어떠해야 근거 있다고 할 수 있는지를, 7장은 증거가 어떠해야 믿을 만한지를, 9장은 실험과 피드백이 어떻게 해야 발산하지 않고 오래 돌아가는지를 다룹니다.

[^ch1-discovery-loop]: Discovery Loop는 2026년 8월 5일 Jeff Dean, Sanjay Ghemawat, Quoc Le, Oriol Vinyals가 설립을 발표한 공익 법인으로, 기계 학습·과학·공학의 자동화를 사명으로 내걸었습니다. 공개된 설명은 완전한 실험 루프를 자동화하고 원래 직렬로 돌던 실험을 대규모로 병렬화한다는 것입니다. https://techcrunch.com/2026/08/05/jeff-dean-and-other-top-ai-researchers-are-leaving-google-to-launch-their-own-startup/ 참조. 이 책을 쓰는 시점까지 재현 가능한 기술적 결과는 발표되지 않았습니다.
...
**다섯 가지 패턴이 책 전체를 관통합니다**: 제안자-검토자, 점진적 공개, 추가만 하기, 경계 집합 + 보존 집합, 최소 diff + 롤백 가능. 모두 같은 모티프, 곧 판단을 모델 자신에게서 모델 바깥의 메커니즘으로 옮기는 것을 공유합니다. 이후의 장들은 이름으로 호출하고 다시 유도하지 않습니다.

**발견 루프는 세 장에 걸칩니다**: 증거(7장의 실패 귀인) → 제안(3장의 지식 갱신) → 실험과 피드백(9장의 검증과 배포). 셋이 쓰는 메커니즘은 본래 다르며, 공유하는 것은 용어가 아니라 위치입니다. 순수한 연구 루프에 비해 에이전트 시스템의 이 루프에는 두 제약이 더 붙습니다. 실험이 진짜 관측에 뿌리내려야 하고, 매 회차가 "무엇을 고쳤고 무엇을 망가뜨렸는지"에 함께 답해야 한다는 것입니다.

**보안은 아키텍처의 문제입니다**: 가드레일, 사람 참여형 개입, 모델 행동을 사람의 의도에 맞추는 정렬은 출시 직전에 덧붙이는 것이 아니라 첫 코드 줄부터 설계해야 합니다. 가드레일은 우회하기 어려운 정도에 따라 컨텍스트 층·실행 층·데이터 층의 세 층으로 나뉘며, 이후 각 장의 보안 논의는 모두 이 골격에 걸립니다.
