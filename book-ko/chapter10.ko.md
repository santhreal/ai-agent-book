...
**교차 검증(Cross-validation)**은 이 사슬을 끊는 핵심 수단입니다. 핵심은 동일한 사고 사슬에 더 많은 에이전트를 참여시키는 것이 아니라, 특정 에이전트가 **독립적인 시각**으로 결론을 재검토하게 하는 것입니다. 즉, 이전 에이전트의 사고 과정을 보지 않고 원본 증거와 최종 결론이 일치하는지만 확인합니다. 이는 5장에서 다룬 제안자-검토자 메커니즘을 멀티 에이전트 시나리오로 확장한 것입니다. 검토자의 가치는 코드 오류나 형식 문제를 찾는 것뿐만 아니라, 독립적인 판단자로서 전체 사고 사슬에서 집단적으로 간과된 모순을 식별하는 데 있습니다. 고위험 의사결정의 경우 외부 검증 수단을 도입할 수도 있습니다.

### 실패 모드 3: 조기 종료와 폭주 루프

조기 종료의 반대편에는 **통제되지 않는 루프**가 있다. 루프는 무한히 실행되거나 토큰 예산을 소진할 수 있다. 실행을 제한된 범위에 두려면 명시적인 예산, 취소 메커니즘, 중지 조건이 필요하다.

### 실패 모드 4: 이해 부채와 인지적 항복
