...
최종 결과만으로는 부족합니다. 유효하고 권한 있는 동작의 비율, 도구 인자의 의미적 정확성, 경로 효율(단계·중복·되돌림), 검색 범위, 비용과 지연을 추적해야 실패 지점을 알 수 있습니다. 민감한 작업, 데이터 유출, 금지 콘텐츠에는 **무관용 원칙**을 적용합니다. 강건성은 seed, UI 변화, API 변동, 오래된 메모리 간섭을 다루며, Agent의 **trajectory**와 실제 시스템 **outcome**을 함께 검증해야 합니다.
...
### 실패 귀속과 trajectory-prefix 회귀 작업
