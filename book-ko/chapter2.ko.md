...
실험 2-7은 직관을 보여 주는 소규모 정성적 데모입니다. “미리 계산해 두고 곧바로 확인하는” 방식이 얼마나 유용하고 한계가 어디인지 저자와 공동 연구자들은 전용 벤치마크로 정량화했습니다[^ch2-8](이 방식의 통칭은 **컨텍스트 증류, Context Distillation**이며 에이전트 상태 표시줄은 가장 일상적인 형태입니다). 결론은 다음과 같습니다.

- **미리 계산한 상태 표시줄**을 제공하면 **약한 모델은 정확도를 회복합니다**. 가장 약한 모델들은 정확도가 40~54%포인트 올랐고, 로컬 2B 모델이 이런 작업에서 상태 표시줄 없는 최첨단 모델과 맞먹기도 했습니다.
- **강한 모델은 원래 정답을 내며, 절약되는 것은 효율입니다**. 같은 상태 표시줄을 쓰면 질의당 사고량, 지연 시간, 비용이 각각 약 한 자릿수 줄어듭니다(사고 토큰은 80~90% 이상 감소).
- 가장 본질적인 변화는 상태 표시줄이 없을 때 질의당 사고량이 컨텍스트 길이에 따라 **계속 증가**하지만, 상태 표시줄이 있으면 **거의 일정**해진다는 점입니다. 컨텍스트가 아무리 길어도 모델은 몇 개의 상태 칸을 “힐끗” 보면 됩니다.


다만 “미리 계산”을 올바르게 했을 때와 잘못했을 때의 차이는 매우 큽니다. 세 가지 교훈이 있습니다.
...
**둘째, 원래 컨텍스트를 삭제하지 마세요.** 상태 표시줄은 원래 컨텍스트의 **손실 투영**으로, 질문받을 것으로 예상한 차원만 미리 계산합니다. 계산이나 상태 추적처럼 상태 표시줄만으로 충분한 작업이라면 원래 기록을 지우고 토큰을 크게 아낄 수 있습니다. 하지만 상태 표시줄이 계산하지 않은 차원을 묻는 질문이 하나라도 나오면, 상태 표시줄만 남겼을 때의 정확도는 급격히 무너집니다.

**셋째, 상태 표시줄 정확도를 최우선 운영 지표로 감시하세요.** 실험에 따르면 **모델은 상태 표시줄을 거의 무조건 신뢰합니다**. “3번 전화했다”고 쓰면 확인하거나 다시 세지 않고 3번으로 받아들입니다. 이것이 상태 표시줄이 효과적인 이유지만, 잘못된 정보도 최종 답변에 **그대로** 전달됩니다. 따라서 앞서 언급한 **상태 표시줄 오염** 위험을 진지하게 다뤄야 합니다.
...
지금까지 압축의 두 목적, 즉 길이 통제와 사고 품질 향상, 그리고 “컨텍스트 내 학습은 본질적으로 검색”이라는 내부 메커니즘을 분석했습니다. 이를 바탕으로 구체적인 압축 전략을 안내할 네 가지 원칙을 정리할 수 있습니다. 여기서 설명하는 압축은 현재 작업을 위한 것입니다. 여러 작업의 궤적을 오프라인에서 지속적인 경험으로 통합한다면 9장에서 다룰 지속적 진화의 문제가 됩니다.
