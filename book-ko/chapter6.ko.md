...
이 장이 **비동기와 이벤트 기반**으로 시작하는 것은 그것이 독자에게 가장 가깝기 때문입니다. 모달리티는 여전히 순수 텍스트이고 바뀌는 것은 타이밍뿐입니다. 앞의 다섯 장이 그린 턴제 세계에서 내딛는 첫걸음이자, "턴제 가정"이라는 명제가 처음으로 땅에 닿는 지점입니다.

## 비동기와 이벤트 기반: 세계가 먼저 찾아올 때

4장에서 다룬 인식·실행·협업 세 범주의 도구는 모두 에이전트가 스스로 호출합니다. 언제 볼지, 언제 움직일지를 결정하는 것은 에이전트입니다. 이 절은 타이밍 축의 가장 느린 쪽으로 옮겨 갑니다. 에이전트는 몇 시간, 때로는 며칠이 걸리는 과제를 어떻게 관리하고, 언제 도착할지 모르는 외부 이벤트에 어떻게 응답할까요? 여기에는 이벤트 기반 비동기 아키텍처의 뒷받침이 필요합니다. 1장의 다섯 범주 가운데 남은 둘, 즉 이벤트 트리거 도구와 사용자 커뮤니케이션 도구가 바로 이 아키텍처에 기대어 작동하므로 이 절에서 함께 다룹니다.
...
[^ch6-async-1]: 약 200줄의 오케스트레이션으로 기성 사고 모델을 연속 시간 에이전트로 바꿀 수 있다는 주장과 “훈련 신호가 지속적 사고의 유용성을 결정한다”라는 결론은 Li, Bojie and Noah Shi. *Never Stop Thinking: Continuous-Time Language Agents.* 2026(출간 예정)에서 가져왔습니다.
...
>
> 마이크, Silero VAD, 로컬 Whisper, 스트리밍 LLM, Fish S1 TTS를 WebSocket으로 연결해 캐스케이드 기준선을 만든다. 실제 단일 턴 증거는 미디어와 모델 체인이 끝까지 실행됐음을 보여 주지만 동시성·운영 부하 벤치마크는 아니다. 코드는 [chapter6/live-audio](../chapter6/live-audio/)에 있다.

> **추가 프로젝트: WebRTC로 “사용자에게 전화하는” 음성 Agent**
>
> PSTN 없이 브라우저 WebRTC로 세션을 열고 부족한 정보를 묻고, 되풀이해 확인한 뒤 구조화된 결과를 저장하는 폐루프를 재현할 수 있다. 외부 기관에는 준수 가능한 PSTN/SIP 제공자로 같은 도구 계약을 교체한다. 전체 경로와 direct/ReAct 비교 증거는 [chapter6/phone-agent](../chapter6/phone-agent/)에 있다. 역사적인 exp9-2 식별자는 유지하지만 본문 번호에는 포함하지 않는다.
...
>
> 증가하는 오디오 프리픽스로 연속 지각을 시뮬레이션하고 600 ms VAD + Whisper와 비교한다. canonical run은 모든 게이트를 통과했지만 기대 동작은 2/6만 재현했다. 호출은 8.4–11.3초가 걸렸고 pause에서 silence를 놓쳤으며 noise에서 cough/laughter를 오분류했다. 이는 실패 방식의 검사이며 100–200 ms 진짜 스트리밍을 주장하지 않는다. 기록은 [chapter6/streaming-speech](../chapter6/streaming-speech/)에 있다.
...
> 표 6-1 MiniCPM-o 4.5 로컬 실행: 엔드투엔드와 자기 캐스케이드 결과(벤치마크가 아니라 4개의 메커니즘 점검)
>
>
...
>
> 표본이 작으므로 전체 정확도나 속도를 결론낼 수 없다. 전체 증거는 [chapter6/end-to-end-speech](../chapter6/end-to-end-speech/)에 있다.
...
MGRD는 음향 특징에 사고를 정박하고 MPS 듀얼 브레인은 계획과 표현을 병렬화한다. 계획 뇌가 사고 조각을 내보내고 표현 뇌가 부분 답변과 합쳐 즉시 음성화하므로 전체 추론을 기다리지 않는다(그림 6-11). 통합형은 자연스럽지만 공동 재훈련이 필요하고 분리형은 백그라운드 모델을 바꾸기 쉽다.

![그림 6-11: Step-Audio R1 MGRD와 MPS 듀얼 브레인 구조](images/fig6-11.svg)
...
![그림 6-12: 컴퓨터 사용 에이전트의 인식-사고-행동 루프](images/fig6-12.svg)

이 루프에는 세 가지 핵심 설계 차원이 있습니다. **행동 공간**(에이전트가 수행할 수 있는 작업), **시각적 그라운딩**(스크린샷에서 대상 요소를 찾는 방법), **모델 아키텍처**(스크린샷에서 올바른 행동을 생성하는 방법)입니다.

### 행동 공간 설계

Anthropic은 완전한 상호작용 능력을 이루는 세 가지 도구 유형을 정의합니다(그림 6-12).

![그림 6-13: 컴퓨터 사용 행동 공간](images/fig6-13.svg)
...
![그림 6-14: Set-of-Mark와 구조화된 요소 색인 비교(browser-use 구현)](images/fig6-14.svg)
...
![그림 6-15: 해상도 매칭과 양방향 좌표 스케일링](images/fig6-15.svg)
...
[^ch6-9]: 게이트 키프레임, 주문형 전사, 프레임을 영속 텍스트로 설명하는 세 구성 요소의 전체 메커니즘과 모델별 제거 실험은 Bojie Li and Noah Shi. *Agent-Computer Observation Interfaces Enable Dynamic Computer Use.* arXiv:2606.29472, 2026을 참고하십시오.
...
짧은 청크는 반응이 빠른 대신 모델 호출이 늘고, 긴 청크는 매끄러운 대신 낡은 관측을 쓰기 쉽다. 실험 6-12은 이런 절충을 시뮬레이터에서 비교하고, 실제 하드웨어의 안전 경계에 닿는 것은 실험 6-11 쪽이다.
...
이 장이 걷어낸 것은 앞의 다섯 장이 줄곧 전제해 온 가정, 곧 에이전트와 세계가 번갈아 발언한다는 가정입니다.

**모달리티**와 **타이밍**이라는 두 축을 따라 보면, 네 절은 같은 명제가 네 가지 시간 척도에서 펼쳐진 것입니다. **비동기와 이벤트 기반**은 관찰을 "에이전트가 능동적으로 가져오는 것"에서 "세계가 밀어 넣는 것"으로, 행동을 "턴 안에서 끝내는 것"에서 "먼저 시작하고 이후는 이벤트로 마무리하는 것"으로 확장합니다. 모달리티는 그대로이고 바뀌는 것은 타이밍뿐입니다. **음성**은 척도를 밀리초까지 압축하며, 캐스케이드·엔드투엔드 Omni·전이중이라는 세 패러다임의 진화 본류는 "번갈아 말하기"에서 지속적인 듣기와 말하기로 나아가고, 전경의 실시간 상호작용과 후경의 깊은 사고 사이에 분업을 이루는 과정이었습니다. **Computer Use**는 같은 폐루프를 화면 위로 옮겨 놓았고, 병목은 "과제를 완수할 수 있는가"에서 조작 효율, 연속적 시각 이해, 행동 후 상태 확인으로 넓어졌습니다. **로봇**은 이를 물리 세계로 밀고 나가며, 행동 청킹은 부드러움과 반응 속도 사이에서 선택을 요구하고, 최종적으로 완수되었는지는 여전히 새로운 관찰로 판정해야 합니다.
...
원시 요소 집합(깨우기, 안전 지점, 취소, 선점, 빠름/느림 분리)도 공유합니다. 이벤트 루프의 "안전 지점에서 취소 신호를 확인한다"와 행동 청킹의 "이상을 발견하면 남은 행동을 버리고 다시 관찰한다"는 다섯 자릿수 차이의 시간 척도에서 같은 메커니즘을 두 번 구현한 것이고, 전경의 빠른 모델과 후경의 느린 모델이라는 분업과 비동기 에이전트의 "먼저 과제 ID를 돌려주고 나중에 이벤트가 마무리한다"도 한 가지 일의 두 가지 표기입니다.

유의할 점은 이 메커니즘들이 아직 대부분 공학적 임시방편이라는 것입니다. 플레이스홀더 프로토콜, 상태 표시줄의 이벤트 표식, 선행 사고의 취소와 롤백은 본질적으로 모델의 훈련 분포에 없는 비동기 경험을 오케스트레이션 로직으로 메우는 일입니다. 모델 쪽의 진전은 그 일부를 파라미터 안으로 거두어들이고 있습니다. 상호작용 모델은 중단과 끼어들기를 모델 내부에 넣었고, 지속적 사고는 "들으면서 생각하기"를 다음 세대 모델을 기다리지 않고도 가능하게 했습니다. 그러나 훈련 말뭉치가 여전히 턴제 중심인 한 이 보상 계층은 사라지지 않으며, 모델 능력이 나아감에 따라 새로운 경계로 계속 옮겨 갈 뿐입니다.
