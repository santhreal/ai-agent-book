...
이로써 이 책의 "에이전트 구축" 부분을 마쳤으며, 코드 생성은 그중 가장 범용적인 메타 역량입니다. 하지만 한 가지 핵심 질문은 아직 답하지 않았습니다. 이러한 설계 결정의 효과를 어떻게 과학적으로 측정할 수 있을까요? 다음 장부터는 "평가와 진화"로 넘어갑니다. 7장에서는 평가 환경, 데이터셋, 자동 판단, 모델 선택을 아우르는 방법론을 개발하고, 8장과 9장에서는 각각 매개변수 수준과 에이전트 시스템 전체의 지속적 개선을 논의합니다.
